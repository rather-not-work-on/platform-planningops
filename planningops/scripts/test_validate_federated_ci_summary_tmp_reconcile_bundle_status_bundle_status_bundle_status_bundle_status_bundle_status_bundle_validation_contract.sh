...
RESOLVED_BUNDLE_PATH="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle.json"
RESOLVED_BUNDLE_VALIDATION_PATH="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-validation.json"
CONFLICT_BUNDLE="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-conflict.json"
CONFLICT_VALIDATION="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-conflict-validation.json"
...
cat >"$REPORT_PATH" <<'JSON'
{
  ...
  "run_id": "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-validator-sample",
...
cat >"$VALIDATION_PATH" <<'JSON'
{
  ...
  "reconcile_run_id": "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-validator-sample",
...
python3 "$ROOT_DIR/scripts/resolve_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --artifact-file "$NEXT_DOCTOR_STATUS_PATH" \
  --output "$RESOLVED_BUNDLE_PATH" >/dev/null

python3 "$ROOT_DIR/scripts/validate_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle.py" \
  --bundle-file "$RESOLVED_BUNDLE_PATH" \
  --output "$RESOLVED_BUNDLE_VALIDATION_PATH" \
  --strict

python3 - <<'PY' "$RESOLVED_BUNDLE_PATH" "$RESOLVED_BUNDLE_VALIDATION_PATH"
import json
import sys
from pathlib import Path
...
assert report["run_id"] == "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-validator-sample", report
...
python3 - <<'PY' "$RESOLVED_BUNDLE_PATH" "$CONFLICT_BUNDLE"
import json
import sys
from pathlib import Path
...
if [[ "$rc" -eq 0 ]]; then
  echo "expected tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status bundle validator to fail on drifted bundle_validation_state"
  exit 1
fi
...
assert any(
    ...
    or "bundle does not match canonical tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status resolution" in error
    for error in report["errors"]
), report
PY

echo "validate federated ci summary tmp reconcile bundle status bundle status bundle status bundle status bundle status bundle ok"
