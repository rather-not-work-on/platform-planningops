...
import validate_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle as bundle_validation
import validate_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status as status_validation
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Print plain python compatibility manifest status-bundle-status-bundle-status-bundle status"
    )
...
def build_status_report(
    ...
    if not ready:
        next_step = (
            "python3 planningops/scripts/doctor_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle.py --bundle-file "
            f"{bundle_path} --require-pass"
        )

    return {
        "bundle_path": str(bundle_path.resolve()),
        "resolved_status_bundle_status_bundle_status_bundle_path": str(status_output.resolve()),
...
def render_lines(kind: str, bundle_path: Path, bundle_doc: dict, validation_report: dict, status_report: dict) -> list[str]:
    ...
    return [
        ...
        f"bundle status output path: {status_report.get('bundle_status_bundle_status_bundle_output_path') or 'missing'}",
        f"bundle status validation output path: {status_report.get('bundle_status_bundle_status_bundle_validation_output_path') or 'missing'}",
