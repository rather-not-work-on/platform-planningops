...
DEFAULT_STATUS_SCHEMA = (
    WORKSPACE_ROOT
    / "planningops/schemas/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status.schema.json"
)
DEFAULT_STATUS_VALIDATION_SCHEMA = (
    WORKSPACE_ROOT
    / "planningops/schemas/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.schema.json"
)
DEFAULT_BUNDLE_SCHEMA = (
    WORKSPACE_ROOT
    / "planningops/schemas/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle.schema.json"
)
DEFAULT_OUTPUT = (
    WORKSPACE_ROOT
...
def resolve_from_status(
    ...
    *,
    ...
    validate_doc_schema(
        status_doc,
        status_schema_path,
        "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status report",
    )
    ...
    if embedded_status_path != status_path:
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status report output_path must match the resolved status path"
        )
    ...
    if status_validation_path is None:
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status report missing status_validation_output_path"
        )
    status_validation_doc = load_json(status_validation_path)
    validate_doc_schema(
        status_validation_doc,
        status_validation_schema_path,
        "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report",
    )

    if resolve_doc_path(status_validation_doc.get("status_path"), base=status_validation_path.parent) != status_path:
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report status_path must match the resolved status path"
        )
    if resolve_doc_path(
        ...
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report status_output_path must match the resolved status path"
        )
    if resolve_doc_path(status_validation_doc.get("output_path"), base=status_validation_path.parent) != status_validation_path:
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report output_path must match the resolved status-validation path"
        )
    if resolve_doc_path(
        ...
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report status_validation_output_path must match the resolved status-validation path"
        )
    if status_validation_doc.get("status_run_id") != status_doc.get("run_id"):
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report must match status run_id"
        )
    if status_validation_doc.get("status_reconcile_status") != status_doc.get("reconcile_status"):
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report must match status reconcile_status"
        )
    if status_validation_doc.get("status_check_name") != status_doc.get("check_name"):
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report must match status check_name"
        )
    if status_validation_doc.get("status_reconcile_count") != status_doc.get("reconcile_count"):
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report must match status reconcile_count"
        )
    if (
        ...
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report must match status reconcile_validation_verdict"
        )
    if status_validation_doc.get("status_bundle_validation_verdict") != status_doc.get("bundle_validation_verdict"):
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report must match status bundle_validation_verdict"
        )
    if status_validation_doc.get("status_bundle_validation_state") != status_doc.get("bundle_validation_state"):
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report must match status bundle_validation_state"
        )
    if status_validation_doc.get("status_ready") != status_doc.get("ready"):
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report must match status ready"
        )
    if status_validation_doc.get("status_next_step") != status_doc.get("next_step"):
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report must match status next_step"
        )
...
def resolve_from_status_validation(
    ...
    *,
    ...
    if status_path is None:
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report missing status_output_path"
        )
    if resolve_doc_path(status_validation_doc.get("status_path"), base=status_validation_path.parent) != status_path:
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report status_path must match status_output_path"
        )
    ...
    if resolve_doc_path(
        ...
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status report status_validation_output_path must match the resolved status-validation path"
        )
    if status_validation_doc.get("status_run_id") != status_doc.get("run_id"):
        raise SystemExit(
            "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report must match status run_id"
...
def resolve_status_bundle_status_bundle_status_bundle_status_bundle_status(
    *,
    ...
    if is_status_doc(artifact_doc):
        ...
        raise SystemExit(
            "artifact must be a tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status report or status-validation report"
        )
...
def main() -> int:
    args = parse_args()
    output_path, bundle = resolve_status_bundle_status_bundle_status_bundle_status_bundle_status(
