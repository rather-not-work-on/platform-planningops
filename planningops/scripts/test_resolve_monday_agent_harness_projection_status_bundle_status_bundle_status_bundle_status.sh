...
NEXT_BUNDLE_FROM_VALIDATION="${TMP_DIR}/projection-status-bundle-status-bundle-status-bundle-status-bundle-from-validation.json"
BUNDLE_SCHEMA="${ROOT_DIR}/planningops/schemas/monday-agent-harness-projection-status-bundle-status-bundle-status-bundle-status-bundle.schema.json"
BROKEN_STATUS="${TMP_DIR}/broken-status-bundle-status-bundle-status-bundle-status.json"
BROKEN_STATUS_VALIDATION="${TMP_DIR}/broken-status-bundle-status-bundle-status-bundle-status-validation.json"
...
python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status.py" \
  --status-validation-file "${OUTER_STATUS_VALIDATION}" \
  --output "${NEXT_BUNDLE_FROM_VALIDATION}" >/dev/null

python3 - <<'PY' "${ROOT_DIR}" "${NEXT_BUNDLE}" "${NEXT_BUNDLE_FROM_VALIDATION}" "${BUNDLE_SCHEMA}" "${OUTER_STATUS}" "${OUTER_STATUS_VALIDATION}" "${OUTER_BUNDLE}"
import json
import sys
...
assert bundle["bundle_status_bundle_status_bundle_status_path"] == str(status_path.resolve()), bundle
assert bundle["bundle_status_bundle_status_bundle_validation_path"] == str(status_validation_path.resolve()), bundle
assert bundle["bundle_path"] == str(outer_bundle_path.resolve()), bundle
assert bundle["resolved_status_bundle_status_bundle_status_bundle_path"] == str(outer_bundle_path.resolve()), bundle
...
assert bundle["resolved_status_bundle_status_bundle_status_bundle_status_bundle_path"] == str(bundle_path.resolve()), bundle
assert bundle["bundle_status_bundle_status_bundle_report"]["bundle_status_bundle_status_bundle_validation_output_path"] == str(status_validation_path.resolve()), bundle
assert bundle["bundle_status_bundle_status_bundle_validation_report"]["bundle_status_bundle_status_bundle_output_path"] == str(status_path.resolve()), bundle
assert validation_bundle["resolved_status_bundle_status_bundle_status_bundle_status_bundle_path"] == str(validation_bundle_path.resolve()), validation_bundle

bundle_without_output = dict(bundle)
bundle_without_output.pop("resolved_status_bundle_status_bundle_status_bundle_status_bundle_path")
validation_bundle_without_output = dict(validation_bundle)
validation_bundle_without_output.pop("resolved_status_bundle_status_bundle_status_bundle_status_bundle_path")
assert bundle_without_output == validation_bundle_without_output, (bundle_without_output, validation_bundle_without_output)
PY

python3 - <<'PY' "${OUTER_STATUS}" "${BROKEN_STATUS}"
import json
import sys
from pathlib import Path

doc = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
doc["bundle_status_bundle_status_bundle_validation_output_path"] = str(
    Path(sys.argv[2]).with_name("missing-status-bundle-status-bundle-status-bundle-status-validation.json").resolve()
)
Path(sys.argv[2]).write_text(json.dumps(doc, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
PY

if python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status.py" \
  --artifact-file "${BROKEN_STATUS}" >/dev/null 2>&1; then
  echo "expected monday projection outer status-bundle resolver to fail on mismatched bundle_status_bundle_status_bundle_validation_output_path" >&2
  exit 1
fi

python3 - <<'PY' "${OUTER_STATUS_VALIDATION}" "${BROKEN_STATUS_VALIDATION}"
import json
import sys
from pathlib import Path

doc = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
doc["bundle_status_bundle_status_bundle_output_path"] = str(
    Path(sys.argv[2]).with_name("missing-status-bundle-status-bundle-status-bundle-status.json").resolve()
)
Path(sys.argv[2]).write_text(json.dumps(doc, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
PY
...
if python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status.py" \
  ...
  echo "expected monday projection outer status-bundle resolver to fail on mismatched bundle_status_bundle_status_bundle_output_path" >&2
  exit 1
fi
...
doc = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
doc["bundle_validation_verdict"] = "fail"
Path(sys.argv[2]).write_text(json.dumps(doc, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
PY

if python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status.py" \
  --status-validation-file "${BROKEN_STATUS_VALIDATION}" >/dev/null 2>&1; then
  echo "expected monday projection outer status-bundle resolver to fail on mismatched bundle_validation_verdict" >&2
  exit 1
fi

echo "resolve monday agent harness projection status bundle status bundle status bundle status ok"
