...
def validate_semantics(status_path: Path, doc: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    bundle_status_output_path = doc.get("bundle_status_bundle_output_path")
    bundle_status_validation_output_path = doc.get("bundle_status_bundle_validation_output_path")
    bundle_validation_output_path = doc.get("bundle_validation_output_path")
    status_bundle_path = doc.get("status_bundle_path")
    outer_status_path = doc.get("bundle_status_path")
    outer_status_validation_path = doc.get("bundle_status_validation_path")
    ...
    if bundle_status_output_path != str(status_path.resolve()):
        append_unique(errors, "bundle_status_bundle_output_path must match the validated status file path")
    if not isinstance(bundle_status_validation_output_path, str) or not bundle_status_validation_output_path.strip():
        append_unique(errors, "bundle_status_bundle_validation_output_path must be a non-empty string")
    ...
    for key, value in (
        ...
        ("bundle_status_path", outer_status_path),
        ("bundle_status_validation_path", outer_status_validation_path),
    ...
    if isinstance(bundle_status_output_path, str) and isinstance(resolved_bundle_path, str) and bundle_status_output_path != resolved_bundle_path:
        append_unique(errors, "resolved_status_bundle_status_bundle_path must match bundle_status_bundle_output_path")
    ...
    if isinstance(validation_doc, dict):
        ...
        if status_bundle_path != validation_doc.get("resolved_status_bundle_path"):
            append_unique(
                errors,
                "status_bundle_path must match bundle-validation report resolved_status_bundle_path",
            )
        if outer_status_path != validation_doc.get("bundle_status_path"):
            append_unique(errors, "bundle_status_path must match bundle-validation report bundle_status_path")
        if outer_status_validation_path != validation_doc.get("bundle_status_validation_path"):
            append_unique(
                errors,
        ...
        if resolved_guardrail_script_paths != validation_doc.get("resolved_guardrail_script_paths"):
            append_unique(
                errors,
                "resolved_guardrail_script_paths must match bundle-validation report resolved_guardrail_script_paths",
            )
...
def build_validation_report(status_path: Path, schema_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    if doc.get("bundle_status_bundle_validation_output_path") != str(output_path.resolve()):
        append_unique(errors, "bundle_status_bundle_validation_output_path must match the validation output path")
        verdict = "fail"
    ...
    return {
        ...
        "bundle_status_bundle_output_path": doc.get("bundle_status_bundle_output_path"),
        "bundle_status_bundle_validation_output_path": doc.get("bundle_status_bundle_validation_output_path"),
        "bundle_validation_output_path": doc.get("bundle_validation_output_path"),
        "status_bundle_path": doc.get("status_bundle_path"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate plain python compatibility manifest status-bundle-status-bundle status report."
    )
