...
import validate_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle as bundle_validation
import validate_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status as status_validation
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "status-bundle-status-bundle-status-bundle-status-bundle-status-bundle health"
        )
    )
...
def doctor_errors(bundle_doc: dict, validation_report: dict) -> list[str]:
    ...
    if bundle_doc.get("ready") is not True:
        append_unique(
            errors,
            "resolved status bundle status bundle status bundle status bundle status bundle ready is false",
        )
    ...
    if bundle_doc.get("next_step") != "none":
        append_unique(
            errors,
            "resolved status bundle status bundle status bundle status bundle status bundle next_step is not none",
        )
    return errors
    ...
    *,
    ...
    next_step = (
        ...
        if ready
            ...
            or "inspect monday projection status bundle status bundle status bundle status bundle status bundle"
        )
    )
    return {
