...
STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle.json"
STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-validation.json"
NEXT_DOCTOR_STATUS_PATH="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status.json"
NEXT_DOCTOR_STATUS_VALIDATION_PATH="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.json"
...
cat >"$REPORT_PATH" <<'JSON'
{
  ...
  "run_id": "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-sample",
...
cat >"$VALIDATION_PATH" <<'JSON'
{
  ...
  "reconcile_run_id": "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-sample",
...
python3 "$ROOT_DIR/scripts/resolve_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status.py" --artifact-file "$DOCTOR_STATUS_PATH" --output "$STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH" >/dev/null
python3 "$ROOT_DIR/scripts/validate_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle.py" --bundle-file "$STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH" --output "$STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH" --strict >/dev/null
python3 "$ROOT_DIR/scripts/doctor_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle.py" --artifact-file "$DOCTOR_STATUS_PATH" --bundle-output "$STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH" --bundle-validation-output "$STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH" --status-output "$NEXT_DOCTOR_STATUS_PATH" --status-validation-output "$NEXT_DOCTOR_STATUS_VALIDATION_PATH" --require-pass >/dev/null

python3 "$ROOT_DIR/scripts/validate_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --status-file "$NEXT_DOCTOR_STATUS_PATH" \
  --bundle-file "$STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH" \
  --bundle-validation-report "$STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH" \
  --output "$NEXT_DOCTOR_STATUS_VALIDATION_PATH" \
  --strict

python3 - <<'PY' "$NEXT_DOCTOR_STATUS_PATH" "$NEXT_DOCTOR_STATUS_VALIDATION_PATH"
import json
import sys
from pathlib import Path
...
assert report["status_path"] == str(Path(sys.argv[1]).resolve()), report
assert report["status_output_path"] == str(Path(sys.argv[1]).resolve()), report
assert report["status_validation_output_path"] == str(Path(sys.argv[2]).resolve()), report
assert report["status_run_id"] == "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-sample", report
...
python3 - <<'PY' "$NEXT_DOCTOR_STATUS_PATH"
import json
import sys
from pathlib import Path
...
if [[ "$rc" -eq 0 ]]; then
  echo "expected strict failure for invalid tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status"
  exit 1
fi
...
assert any(
    "ready must match canonical status-bundle-status bundle status bundle status bundle status doctor status" in item
    for item in report["errors"]
), report
assert any(
    "next_step must match canonical status-bundle-status bundle status bundle status bundle status doctor status" in item
    for item in report["errors"]
), report
PY

echo "validate federated ci summary tmp reconcile bundle status bundle status bundle status bundle status bundle status contract ok"
