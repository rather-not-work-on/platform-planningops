...
import resolve_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status as bundle_resolver
import validate_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle as bundle_validation


WORKSPACE_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_ARTIFACT = (
    WORKSPACE_ROOT / "artifacts/validation/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status.json"
)
DEFAULT_BUNDLE = (
    WORKSPACE_ROOT
...
DEFAULT_STATUS_SCHEMA = (
    WORKSPACE_ROOT / "schemas/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status.schema.json"
)
DEFAULT_STATUS_VALIDATION_SCHEMA = (
    WORKSPACE_ROOT / "schemas/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-validation.schema.json"
)
...
STATUS_VALIDATOR = (
    WORKSPACE_ROOT / "scripts/validate_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status.py"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Print the latest federated CI tmp-summary reconcile bundle status-bundle-status-bundle-status-bundle health"
    )
...
def load_bundle(args: argparse.Namespace) -> tuple[Path, dict, dict | None]:
    ...
    if args.bundle_file:
        ...
        if isinstance(artifact_file, str) and artifact_file.strip():
            _, canonical_bundle = bundle_resolver.resolve_status_bundle_status_bundle_status(
    ...
    _, bundle_doc = bundle_resolver.resolve_status_bundle_status_bundle_status(
