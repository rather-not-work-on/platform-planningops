...
SCHEMA_PATH="${ROOT_DIR}/planningops/schemas/monday-agent-harness-projection-status-bundle-status-bundle-validation.schema.json"
INVALID_BUNDLE="${TMP_DIR}/invalid-status-bundle-status-bundle.json"
INVALID_BUNDLE_VALIDATION="${TMP_DIR}/invalid-status-bundle-status-bundle-validation.json"
...
python3 "${ROOT_DIR}/planningops/scripts/validate_monday_agent_harness_projection_status_bundle_status_bundle.py" \
  --bundle-file "${STATUS_BUNDLE_STATUS_BUNDLE}" \
  --output "${STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION}" \
  --strict

python3 - <<'PY' "${ROOT_DIR}" "${STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION}" "${SCHEMA_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE}" "${STATUS_BUNDLE_STATUS}" "${STATUS_BUNDLE_STATUS_VALIDATION}" "${STATUS_BUNDLE}"
import json
import sys
...
resolved_status_bundle_path = Path(sys.argv[7])
...
assert report["status_bundle_status_bundle_path"] == str(bundle_path.resolve()), report
assert report["bundle_status_path"] == str(status_path.resolve()), report
assert report["bundle_status_validation_path"] == str(status_validation_path.resolve()), report
assert report["bundle_path"] == str(resolved_status_bundle_path.resolve()), report
...
python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE}" "${INVALID_BUNDLE}"
import json
import sys
from pathlib import Path
...
doc["bundle_status_report"]["bundle_status_validation_output_path"] = str(Path(sys.argv[2]).with_name("wrong-status-bundle-status-validation.json").resolve())
...
if [[ "${rc}" -eq 0 ]]; then
  echo "expected strict failure for invalid monday harness projection status bundle status bundle"
  exit 1
fi
...
assert any("bundle_status_report.bundle_status_validation_output_path must match bundle_status_validation_path" in item for item in report["errors"]), report
assert any("bundle must match canonical monday harness projection status-bundle-status bundle resolution" in item for item in report["errors"]), report
PY

echo "validate monday agent harness projection status bundle status bundle validation contract ok"
