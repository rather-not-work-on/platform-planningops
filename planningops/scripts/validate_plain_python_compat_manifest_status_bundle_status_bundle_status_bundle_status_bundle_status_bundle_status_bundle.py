...
import resolve_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status as bundle_resolver
...
def build_report_payload(bundle_path: Path, bundle_schema_path: Path, doc: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "bundle_status_bundle_status_bundle_status_bundle_status_path": _string(
            doc.get("bundle_status_bundle_status_bundle_status_bundle_status_path"),
            bundle_path_str,
        ),
        "bundle_status_bundle_status_bundle_status_bundle_validation_path": _string(
        ...
        "resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path": _string(
            doc.get("resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path"),
            bundle_path_str,
        ),
...
def validate_semantics(
    ...
    *,
    ...
    status_path = doc.get("bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_path")
    status_validation_path = doc.get("bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_path")
    ready = doc.get("ready")
    ...
    status_report = doc.get("bundle_status_bundle_status_bundle_status_bundle_status_bundle_report")
    if not isinstance(status_report, dict):
        append_unique(errors, "bundle_status_bundle_status_bundle_status_bundle_status_bundle_report must be an object")
    else:
        if ready != status_report.get("ready"):
            append_unique(errors, "ready must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_report.ready")
        if next_step != status_report.get("next_step"):
            append_unique(
                errors,
                "next_step must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_report.next_step",
            )
        if status_verdict != status_report.get("verdict"):
            append_unique(
                errors,
                "status_verdict must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_report.verdict",
            )
        if status_path != status_report.get("bundle_status_bundle_status_bundle_status_bundle_status_bundle_output_path"):
            append_unique(
                errors,
                "bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_path must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_report.bundle_status_bundle_status_bundle_status_bundle_status_bundle_output_path",
            )
        if (
            status_validation_path
            != status_report.get("bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_output_path")
        ):
            append_unique(
                errors,
                "bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_path must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_report.bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_output_path",
            )

    status_validation_report = doc.get(
        "bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_report"
    )
    if not isinstance(status_validation_report, dict):
        append_unique(
            errors,
            "bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_report must be an object",
        )
    else:
        if status_validation_verdict != status_validation_report.get("verdict"):
            append_unique(
                errors,
                "status_validation_verdict must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_report.verdict",
            )
        if ready != status_validation_report.get("status_ready"):
            append_unique(
                errors,
                "ready must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_report.status_ready",
            )
        if next_step != status_validation_report.get("status_next_step"):
            append_unique(
                errors,
                "next_step must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_report.status_next_step",
            )
        if status_verdict != status_validation_report.get("status_verdict"):
            append_unique(
                errors,
                "status_verdict must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_report.status_verdict",
            )
        if (
            status_path
            != status_validation_report.get("bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_path")
        ):
            append_unique(
                errors,
                "bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_path must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_report.bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_path",
            )
        if status_path != status_validation_report.get("bundle_status_bundle_status_bundle_status_bundle_status_bundle_output_path"):
            append_unique(
                errors,
                "bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_path must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_report.bundle_status_bundle_status_bundle_status_bundle_status_bundle_output_path",
            )
        if (
            status_validation_path
            != status_validation_report.get("bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_output_path")
        ):
            append_unique(
                errors,
                "bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_path must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_report.bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_output_path",
            )
        if status_validation_path != status_validation_report.get("output_path"):
            append_unique(
                errors,
                "bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_path must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_report.output_path",
            )

    if isinstance(status_path, str) and status_path.strip():
        try:
            canonical_bundle = bundle_resolver.resolve_status_bundle_status_bundle_status_bundle_status_bundle_status(
                artifact_file=None,
            ...
            if "resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path" in doc:
                expected_bundle[
                    "resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path"
            ...
            if doc != expected_bundle:
                append_unique(
                    errors,
                    "bundle must match canonical status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status bundle resolution",
                )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate plain python compatibility manifest status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status bundle."
    )
