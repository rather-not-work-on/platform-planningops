...
BUNDLE_STATUS_VALIDATION_PATH="${TMP_DIR}/plain-python-compat-status-bundle-produced-validation.json"
BUNDLE_VALIDATION_PATH="${TMP_DIR}/plain-python-compat-status-bundle-validation.json"
BUNDLE_VALIDATION_SCHEMA_PATH="${ROOT_DIR}/planningops/schemas/plain-python-compat-manifest-status-bundle-validation.schema.json"
...
python3 "${ROOT_DIR}/planningops/scripts/doctor_plain_python_compat_manifest.py" \
  ...
  --status-bundle-validation-output "${BUNDLE_STATUS_VALIDATION_PATH}" >/dev/null

python3 "${ROOT_DIR}/planningops/scripts/validate_plain_python_compat_manifest_status_bundle.py" \
  --bundle-file "${STATUS_BUNDLE_PATH}" \
  --output "${BUNDLE_VALIDATION_PATH}" \
  --strict

python3 - <<'PY' "${ROOT_DIR}" "${BUNDLE_VALIDATION_PATH}" "${BUNDLE_VALIDATION_SCHEMA_PATH}" "${STATUS_BUNDLE_PATH}"
import json
import sys
...
python3 - <<'PY' "${STATUS_BUNDLE_PATH}" "${TMP_DIR}/invalid-bundle.json"
import json
import sys
from pathlib import Path
...
doc["resolved_status_bundle_path"] = str(Path(sys.argv[2]).with_name("wrong-bundle.json").resolve())
...
set +e
python3 "${ROOT_DIR}/planningops/scripts/validate_plain_python_compat_manifest_status_bundle.py" \
  --bundle-file "${TMP_DIR}/invalid-bundle.json" \
  --output "${TMP_DIR}/invalid-bundle-validation.json" \
  --strict
rc=$?
set -e

if [[ "${rc}" -eq 0 ]]; then
  echo "expected strict failure for invalid plain python compat manifest status bundle"
  exit 1
fi

python3 - <<'PY' "${TMP_DIR}/invalid-bundle-validation.json"
import json
import sys
from pathlib import Path
...
assert any("bundle must match canonical status bundle resolution" in item for item in report["errors"]), report
PY

echo "validate plain python compat manifest status bundle validation contract ok"
