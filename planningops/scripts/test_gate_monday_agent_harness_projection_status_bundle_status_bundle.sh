...
GATE_BUNDLE="${TMP_DIR}/projection-status-bundle-status-bundle.json"
GATE_BUNDLE_VALIDATION="${TMP_DIR}/projection-status-bundle-status-bundle-validation.json"
GATE_STATUS="${TMP_DIR}/projection-status-bundle-status-bundle-status.json"
GATE_STATUS_VALIDATION="${TMP_DIR}/projection-status-bundle-status-bundle-status-validation.json"
...
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","finalPhase":"publish_evidence","finalStatus":"succeeded","verificationVerdict":"pass","completedAtUtc":"2026-03-23T12:40:00Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/readiness-projection.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","readinessStatus":"ready","reason":"verification_passed","verificationVerdict":"pass","blockingConditions":[],"requiredEvidenceRefs":["${EVIDENCE}"],"generatedAtUtc":"2026-03-23T12:40:01Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/verification-projection.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","verificationVerdict":"pass","verificationReportRefs":["${EVIDENCE}"],"failedChecks":[],"repairAttempts":0,"generatedAtUtc":"2026-03-23T12:40:02Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/operator-handoff-summary.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","finalStatus":"succeeded","verificationVerdict":"pass","handoffStatus":"not_required","handoffReason":"none","nextRequiredActor":"none","recommendedNextStep":"none","blockingQuestionSet":[],"generatedAtUtc":"2026-03-23T12:40:03Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
...
bash "${ROOT_DIR}/planningops/scripts/gate_monday_agent_harness_projection_status_bundle_status_bundle.sh" \
...
doc["next_step"] = "rerun monday projection bundle doctor"
Path(sys.argv[2]).write_text(json.dumps(doc, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
PY

if bash "${ROOT_DIR}/planningops/scripts/gate_monday_agent_harness_projection_status_bundle_status_bundle.sh" \
  --bundle-file "${BROKEN_BUNDLE}" \
  --bundle-validation-output "${GATE_BUNDLE_VALIDATION}" \
  --status-output "${GATE_STATUS}" \
  --status-validation-output "${GATE_STATUS_VALIDATION}" >/dev/null 2>&1; then
  echo "expected monday projection status-bundle-status-bundle gate to fail on broken bundle" >&2
  exit 1
fi

echo "gate monday agent harness projection status bundle status bundle ok"
