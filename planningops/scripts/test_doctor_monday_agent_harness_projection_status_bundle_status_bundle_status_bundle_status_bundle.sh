...
DOCTOR_STATUS="${TMP_DIR}/projection-status-bundle-status-bundle-status-bundle-status-bundle-status.json"
DOCTOR_STATUS_VALIDATION="${TMP_DIR}/projection-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.json"
REPORT_READY="${TMP_DIR}/doctor-status-bundle-status-bundle-status-bundle-status-bundle.txt"
BROKEN_BUNDLE="${TMP_DIR}/broken-status-bundle-status-bundle-status-bundle-status-bundle.json"
REPORT_BROKEN="${TMP_DIR}/doctor-status-bundle-status-bundle-status-bundle-status-bundle-broken.txt"
OUTER_BUNDLE_REAL="$(python3 - <<'PY' "${OUTER_BUNDLE}"
from pathlib import Path
import sys
print(Path(sys.argv[1]).resolve())
PY
)"
OUTER_BUNDLE_VALIDATION_REAL="$(python3 - <<'PY' "${OUTER_BUNDLE_VALIDATION}"
from pathlib import Path
import sys
print(Path(sys.argv[1]).resolve())
PY
...
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","finalPhase":"publish_evidence","finalStatus":"succeeded","verificationVerdict":"pass","completedAtUtc":"2026-03-23T14:30:00Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/readiness-projection.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","readinessStatus":"ready","reason":"verification_passed","verificationVerdict":"pass","blockingConditions":[],"requiredEvidenceRefs":["${EVIDENCE}"],"generatedAtUtc":"2026-03-23T14:30:01Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/verification-projection.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","verificationVerdict":"pass","verificationReportRefs":["${EVIDENCE}"],"failedChecks":[],"repairAttempts":0,"generatedAtUtc":"2026-03-23T14:30:02Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/operator-handoff-summary.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","finalStatus":"succeeded","verificationVerdict":"pass","handoffStatus":"not_required","handoffReason":"none","nextRequiredActor":"none","recommendedNextStep":"none","blockingQuestionSet":[],"generatedAtUtc":"2026-03-23T14:30:03Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
...
grep -q "bundle path: ${OUTER_BUNDLE_REAL}" "${REPORT_READY}"
grep -q "bundle validation output path: ${OUTER_BUNDLE_VALIDATION_REAL}" "${REPORT_READY}"
...
python3 - <<'PY' "${OUTER_BUNDLE}" "${OUTER_BUNDLE_VALIDATION}" "${DOCTOR_STATUS}" "${DOCTOR_STATUS_VALIDATION}"
import json
import sys
from pathlib import Path
...
assert bundle["resolved_status_bundle_status_bundle_status_bundle_status_bundle_path"] == str(Path(sys.argv[1]).resolve()), bundle
assert validation["status_bundle_status_bundle_status_bundle_status_bundle_path"] == str(Path(sys.argv[1]).resolve()), validation
assert validation["verdict"] == "pass", validation
assert doctor_status["bundle_path"] == str(Path(sys.argv[1]).resolve()), doctor_status
assert doctor_status["bundle_status_bundle_status_bundle_status_bundle_output_path"] == str(Path(sys.argv[3]).resolve()), doctor_status
assert doctor_status["bundle_status_bundle_status_bundle_status_bundle_validation_output_path"] == str(Path(sys.argv[4]).resolve()), doctor_status
...
assert doctor_status_validation["bundle_status_bundle_status_bundle_status_bundle_status_path"] == str(Path(sys.argv[3]).resolve()), doctor_status_validation
assert doctor_status_validation["bundle_status_bundle_status_bundle_status_bundle_validation_output_path"] == str(Path(sys.argv[4]).resolve()), doctor_status_validation
...
if python3 "${ROOT_DIR}/planningops/scripts/doctor_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle.py" \
  --bundle-file "${BROKEN_BUNDLE}" \
  --bundle-validation-output "${OUTER_BUNDLE_VALIDATION}" \
  --status-output "${DOCTOR_STATUS}" \
  --status-validation-output "${DOCTOR_STATUS_VALIDATION}" \
  --require-pass >"${REPORT_BROKEN}" 2>&1; then
  echo "expected monday projection status-bundle-status-bundle-status-bundle-status-bundle doctor to fail on broken bundle" >&2
  exit 1
fi
...
echo "doctor monday agent harness projection status bundle status bundle status bundle status bundle ok"
