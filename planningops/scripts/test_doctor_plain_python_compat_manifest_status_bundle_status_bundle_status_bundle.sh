...
REPORT_PATH="${TMP_DIR}/doctor-status-bundle-status-bundle-status-bundle.txt"
BROKEN_BUNDLE_PATH="${TMP_DIR}/broken-status-bundle-status-bundle-status-bundle.json"
BROKEN_REPORT_PATH="${TMP_DIR}/doctor-status-bundle-status-bundle-status-bundle-broken.txt"
STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_REAL="$(python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH}"
...
STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_REAL="$(python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH}"
from pathlib import Path
import sys
...
STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_REAL="$(python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH}"
from pathlib import Path
import sys
...
STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_REAL="$(python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH}"
from pathlib import Path
import sys
...
python3 "${ROOT_DIR}/planningops/scripts/doctor_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle.py" \
  ...
  --bundle-validation-output "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH}" >"${REPORT_PATH}"

grep -q "source kind: artifact" "${REPORT_PATH}"
grep -q "bundle path: ${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_REAL}" "${REPORT_PATH}"
grep -q "bundle status output path: ${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_REAL}" "${REPORT_PATH}"
grep -q "bundle status validation output path: ${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_REAL}" "${REPORT_PATH}"
grep -q "bundle validation output path: ${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_REAL}" "${REPORT_PATH}"
...
python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_PATH}" "${STATUS_BUNDLE_STATUS_PATH}" "${STATUS_BUNDLE_STATUS_VALIDATION_PATH}" "${STATUS_BUNDLE_PATH}"
import json
import sys
from pathlib import Path
...
assert bundle["resolved_status_bundle_status_bundle_status_bundle_path"] == str(Path(sys.argv[1]).resolve()), bundle
assert bundle["bundle_status_bundle_status_path"] == str(Path(sys.argv[5]).resolve()), bundle
assert bundle["bundle_status_bundle_validation_path"] == str(Path(sys.argv[6]).resolve()), bundle
assert bundle["bundle_path"] == str(Path(sys.argv[7]).resolve()), bundle
assert bundle["status_bundle_path"] == str(Path(sys.argv[10]).resolve()), bundle
assert bundle["bundle_status_path"] == str(Path(sys.argv[8]).resolve()), bundle
assert bundle["bundle_status_validation_path"] == str(Path(sys.argv[9]).resolve()), bundle
...
assert status["bundle_status_bundle_status_bundle_output_path"] == str(Path(sys.argv[2]).resolve()), status
assert status["bundle_status_bundle_status_bundle_validation_output_path"] == str(Path(sys.argv[4]).resolve()), status
assert status["bundle_path"] == str(Path(sys.argv[1]).resolve()), status
assert status["bundle_status_bundle_status_path"] == str(Path(sys.argv[5]).resolve()), status
assert status["bundle_status_bundle_validation_path"] == str(Path(sys.argv[6]).resolve()), status
assert status["status_bundle_path"] == str(Path(sys.argv[10]).resolve()), status
assert status["bundle_status_path"] == str(Path(sys.argv[8]).resolve()), status
assert status["bundle_status_validation_path"] == str(Path(sys.argv[9]).resolve()), status
...
assert status_validation["bundle_status_bundle_status_bundle_status_path"] == str(Path(sys.argv[2]).resolve()), status_validation
assert status_validation["bundle_status_bundle_status_bundle_validation_output_path"] == str(Path(sys.argv[4]).resolve()), status_validation
assert status_validation["verdict"] == "pass", status_validation
PY

python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH}" "${BROKEN_BUNDLE_PATH}"
import json
import sys
from pathlib import Path
...
if python3 "${ROOT_DIR}/planningops/scripts/doctor_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle.py" \
  ...
  echo "expected status-bundle-status-bundle-status-bundle doctor to fail on broken bundle" >&2
  exit 1
fi
...
echo "doctor plain python compat manifest status bundle status bundle status bundle ok"
