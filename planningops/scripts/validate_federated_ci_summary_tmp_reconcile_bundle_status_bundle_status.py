...
from doctor_federated_ci_summary_tmp_reconcile_bundle_status_bundle import build_status


WORKSPACE_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_STATUS = WORKSPACE_ROOT / "artifacts/validation/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status.json"
...
DEFAULT_SCHEMA = WORKSPACE_ROOT / "schemas/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status.schema.json"
DEFAULT_VALIDATION_SCHEMA = (
    WORKSPACE_ROOT / "schemas/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-validation.schema.json"
)
DEFAULT_OUTPUT = (
    WORKSPACE_ROOT / "artifacts/validation/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-validation.json"
)
    ...
    *,
    ...
    for field in (
        ...
        if status_doc.get(field) != expected_status.get(field):
            append_unique(errors, f"{field} must match canonical status-bundle doctor status")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate federated CI tmp-summary reconcile bundle status-bundle status artifacts."
    )
