...
SCHEMA_PATH="${ROOT_DIR}/planningops/schemas/monday-agent-harness-projection-status-bundle-status-bundle-status-validation.schema.json"
INVALID_STATUS="${TMP_DIR}/invalid-status-bundle-status-bundle-status.json"
INVALID_STATUS_VALIDATION="${TMP_DIR}/invalid-status-bundle-status-bundle-status-validation.json"
...
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","finalPhase":"publish_evidence","finalStatus":"succeeded","verificationVerdict":"pass","completedAtUtc":"2026-03-23T12:20:00Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/readiness-projection.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","readinessStatus":"ready","reason":"verification_passed","verificationVerdict":"pass","blockingConditions":[],"requiredEvidenceRefs":["${EVIDENCE}"],"generatedAtUtc":"2026-03-23T12:20:01Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/verification-projection.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","verificationVerdict":"pass","verificationReportRefs":["${EVIDENCE}"],"failedChecks":[],"repairAttempts":0,"generatedAtUtc":"2026-03-23T12:20:02Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/operator-handoff-summary.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","finalStatus":"succeeded","verificationVerdict":"pass","handoffStatus":"not_required","handoffReason":"none","nextRequiredActor":"none","recommendedNextStep":"none","blockingQuestionSet":[],"generatedAtUtc":"2026-03-23T12:20:03Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
...
python3 "${ROOT_DIR}/planningops/scripts/validate_monday_agent_harness_projection_status_bundle_status_bundle_status.py" \
  --status-file "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS}" \
  --output "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION}" \
  --strict

python3 - <<'PY' "${ROOT_DIR}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION}" "${SCHEMA_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS}" "${STATUS_BUNDLE_STATUS_BUNDLE}"
import json
import sys
...
python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS}" "${INVALID_STATUS}"
import json
import sys
from pathlib import Path
...
doc["bundle_status_bundle_validation_output_path"] = str(Path(sys.argv[2]).with_name("wrong-status-validation.json").resolve())
...
if [[ "${rc}" -eq 0 ]]; then
  echo "expected strict failure for invalid monday harness projection status bundle status bundle status"
  exit 1
fi
...
assert any("projection_validation_state must match resolved bundle projection_validation_state" in item for item in report["errors"]), report
assert any("bundle_status_bundle_validation_output_path must match the validation output path" in item for item in report["errors"]), report
assert any("ready must match bundle-validation report bundle_ready" in item for item in report["errors"]), report
assert any("next_step must match bundle-validation report bundle_next_step" in item for item in report["errors"]), report
PY

echo "validate monday agent harness projection status bundle status bundle status validation contract ok"
