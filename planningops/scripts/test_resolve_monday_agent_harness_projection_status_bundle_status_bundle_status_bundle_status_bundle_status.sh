...
NEXT_BUNDLE_FROM_VALIDATION="${TMP_DIR}/projection-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-from-validation.json"
BUNDLE_SCHEMA="${ROOT_DIR}/planningops/schemas/monday-agent-harness-projection-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle.schema.json"
BROKEN_STATUS="${TMP_DIR}/broken-status-bundle-status-bundle-status-bundle-status-bundle-status.json"
BROKEN_STATUS_VALIDATION="${TMP_DIR}/broken-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.json"
...
assert bundle["resolved_status_bundle_status_bundle_status_bundle_status_bundle_path"] == str(outer_bundle_path.resolve()), bundle
...
doc["bundle_status_bundle_status_bundle_status_bundle_validation_output_path"] = str(
    Path(sys.argv[2]).with_name("missing-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.json").resolve()
)
Path(sys.argv[2]).write_text(json.dumps(doc, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
PY

if python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --artifact-file "${BROKEN_STATUS}" >/dev/null 2>&1; then
  echo "expected monday projection outer status-bundle resolver to fail on mismatched bundle_status_bundle_status_bundle_status_bundle_validation_output_path" >&2
  exit 1
fi
...
doc = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
doc["bundle_status_bundle_status_bundle_status_bundle_output_path"] = str(
    Path(sys.argv[2]).with_name("missing-status-bundle-status-bundle-status-bundle-status-bundle-status.json").resolve()
)
Path(sys.argv[2]).write_text(json.dumps(doc, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
PY
...
if python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  ...
  echo "expected monday projection outer status-bundle resolver to fail on mismatched bundle_status_bundle_status_bundle_status_bundle_output_path" >&2
  exit 1
fi
...
echo "resolve monday agent harness projection status bundle status bundle status bundle status bundle status ok"
