...
bash "${ROOT_DIR}/planningops/scripts/gate_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle.sh" \
  --artifact-file "${OUTER_STATUS}" \
  --bundle-output "${NEW_BUNDLE}" \
  --bundle-validation-output "${NEW_BUNDLE_VALIDATION}" >/dev/null
...
if bash "${ROOT_DIR}/planningops/scripts/gate_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle.sh" \
  --bundle-file "${BROKEN_BUNDLE}" \
  --bundle-validation-output "${NEW_BUNDLE_VALIDATION}" >/dev/null 2>&1; then
  echo "expected monday projection status-bundle-status-bundle-status-bundle-status-bundle-status-bundle gate to fail on broken bundle" >&2
  exit 1
fi

echo "gate monday agent harness projection status bundle status bundle status bundle status bundle status bundle ok"
