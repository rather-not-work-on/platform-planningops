...
STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_FROM_VALIDATION_PATH="${TMP_DIR}/plain-python-compat-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-from-validation.json"
BUNDLE_SCHEMA_PATH="${ROOT_DIR}/planningops/schemas/plain-python-compat-manifest-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle.schema.json"
BROKEN_STATUS_PATH="${TMP_DIR}/broken-status-bundle-status-bundle-status-bundle-status-bundle-status.json"
BROKEN_STATUS_VALIDATION_PATH="${TMP_DIR}/broken-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.json"
...
python3 "${ROOT_DIR}/planningops/scripts/resolve_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --status-validation-file "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH}" \
  --output "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_FROM_VALIDATION_PATH}" >/dev/null

python3 - <<'PY' "${ROOT_DIR}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_FROM_VALIDATION_PATH}" "${BUNDLE_SCHEMA_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH}"
import json
import sys
...
assert bundle["bundle_status_bundle_status_bundle_status_bundle_status_path"] == str(status_path.resolve()), bundle
assert bundle["bundle_status_bundle_status_bundle_status_bundle_validation_path"] == str(status_validation_path.resolve()), bundle
...
assert bundle["resolved_status_bundle_status_bundle_status_bundle_status_bundle_path"] == str(status_path.resolve()), bundle
assert bundle["resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path"] == str(bundle_path.resolve()), bundle
assert bundle["bundle_status_bundle_status_bundle_status_bundle_report"]["bundle_status_bundle_status_bundle_status_bundle_validation_output_path"] == str(status_validation_path.resolve()), bundle
assert bundle["bundle_status_bundle_status_bundle_status_bundle_validation_report"]["bundle_status_bundle_status_bundle_status_bundle_output_path"] == str(status_path.resolve()), bundle
assert validation_bundle["resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path"] == str(validation_bundle_path.resolve()), validation_bundle

bundle_without_output = dict(bundle)
bundle_without_output.pop("resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path")
validation_bundle_without_output = dict(validation_bundle)
validation_bundle_without_output.pop("resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path")
assert bundle_without_output == validation_bundle_without_output, (bundle_without_output, validation_bundle_without_output)
PY

python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH}" "${BROKEN_STATUS_PATH}"
import json
import sys
from pathlib import Path

doc = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
doc["bundle_status_bundle_status_bundle_status_bundle_validation_output_path"] = str(Path(sys.argv[2]).with_name("missing-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.json").resolve())
Path(sys.argv[2]).write_text(json.dumps(doc, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
PY

if python3 "${ROOT_DIR}/planningops/scripts/resolve_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --artifact-file "${BROKEN_STATUS_PATH}" >/dev/null 2>&1; then
  echo "expected outermost-doctor status resolver to fail on mismatched bundle_status_bundle_status_bundle_status_bundle_validation_output_path" >&2
  exit 1
fi

python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH}" "${BROKEN_STATUS_VALIDATION_PATH}"
import json
import sys
from pathlib import Path

doc = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
doc["bundle_status_bundle_status_bundle_status_bundle_output_path"] = str(Path(sys.argv[2]).with_name("missing-status-bundle-status-bundle-status-bundle-status-bundle-status.json").resolve())
Path(sys.argv[2]).write_text(json.dumps(doc, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
PY

if python3 "${ROOT_DIR}/planningops/scripts/resolve_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --status-validation-file "${BROKEN_STATUS_VALIDATION_PATH}" >/dev/null 2>&1; then
  echo "expected outermost-doctor status resolver to fail on mismatched bundle_status_bundle_status_bundle_status_bundle_output_path" >&2
  exit 1
fi

echo "resolve plain python compat manifest status bundle status bundle status bundle status bundle status ok"
