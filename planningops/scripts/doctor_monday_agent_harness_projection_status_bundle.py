...
import validate_monday_agent_harness_projection_status_bundle as bundle_validation
import validate_monday_agent_harness_projection_status_bundle_status as status_validation
...
BASE_DOCTOR = bundle_resolver.WORKSPACE_ROOT / "planningops/scripts/doctor_monday_agent_harness_projection.py"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Print monday agent harness projection resolved status-bundle health"
    )
...
def doctor_errors(bundle_doc: dict, validation_report: dict) -> list[str]:
    ...
    if bundle_doc.get("ready") is not True:
        append_unique(errors, "resolved status bundle ready is false")
    ...
    if bundle_doc.get("next_step") != "none":
        append_unique(errors, "resolved status bundle next_step is not none")
    return errors


def build_status_report(bundle_path: Path, bundle_doc: dict, validation_report: dict, *, status_output: Path, status_validation_output: Path) -> dict:
    errors = doctor_errors(bundle_doc, validation_report)
    warnings = list(validation_report.get("warnings") or [])
    verdict = "pass" if not errors else "fail"
    ready = verdict == "pass"
    next_step = "none" if ready else (bundle_doc.get("next_step") or "inspect monday projection status bundle")
    return {
        "bundle_path": str(bundle_path.resolve()),
        "resolved_status_bundle_path": bundle_doc.get("resolved_status_bundle_path") or str(bundle_path.resolve()),
        ...
        "errors": errors,
        "warnings": warnings,
        "projection_bundle_path": bundle_doc.get("bundle_path"),
        "projection_validation_report_path": bundle_doc.get("validation_report_path"),
...
def render_lines(kind: str, bundle_path: Path, bundle_doc: dict, validation_report: dict) -> list[str]:
    ...
    next_step = "none" if doctor_verdict == "pass" else (bundle_doc.get("next_step") or "inspect monday projection status bundle")
    return [
        f"source kind: {kind}",
        f"bundle path: {bundle_path}",
        f"bundle validation output path: {validation_report.get('output_path') or 'missing'}",
        f"projection bundle path: {bundle_doc.get('bundle_path') or 'missing'}",
        f"projection validation report path: {bundle_doc.get('validation_report_path') or 'missing'}",
...
def main() -> int:
    ...
    if args.require_pass and (doctor_errors(bundle_doc, validation_report) or status_validation_report.get("verdict") != "pass"):
        return 1
    return 0
