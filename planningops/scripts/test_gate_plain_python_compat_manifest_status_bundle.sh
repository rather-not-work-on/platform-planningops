...
GATE_BUNDLE_PATH="${TMP_DIR}/plain-python-compat-status-bundle.json"
GATE_BUNDLE_VALIDATION_PATH="${TMP_DIR}/plain-python-compat-status-bundle-validation.json"
GATE_STATUS_PATH="${TMP_DIR}/plain-python-compat-status-bundle-status.json"
GATE_STATUS_VALIDATION_PATH="${TMP_DIR}/plain-python-compat-status-bundle-status-validation.json"
...
bash "${ROOT_DIR}/planningops/scripts/gate_plain_python_compat_manifest_status_bundle.sh" \
  ...
  --bundle-output "${GATE_BUNDLE_PATH}" \
  --status-output "${GATE_STATUS_PATH}" \
  --status-validation-output "${GATE_STATUS_VALIDATION_PATH}" \
  --bundle-validation-output "${GATE_BUNDLE_VALIDATION_PATH}" >/dev/null

python3 - <<'PY' "${GATE_BUNDLE_PATH}" "${GATE_BUNDLE_VALIDATION_PATH}" "${GATE_STATUS_PATH}" "${GATE_STATUS_VALIDATION_PATH}"
import json
import sys
...
python3 - <<'PY' "${GATE_BUNDLE_PATH}" "${BROKEN_BUNDLE_PATH}"
import json
import sys
from pathlib import Path
...
if bash "${ROOT_DIR}/planningops/scripts/gate_plain_python_compat_manifest_status_bundle.sh" \
  --bundle-file "${BROKEN_BUNDLE_PATH}" \
  --status-output "${GATE_STATUS_PATH}" \
  --status-validation-output "${GATE_STATUS_VALIDATION_PATH}" \
  --bundle-validation-output "${GATE_BUNDLE_VALIDATION_PATH}" >/dev/null 2>&1; then
  echo "expected status bundle gate to fail on broken bundle" >&2
  exit 1
fi

echo "gate plain python compat manifest status bundle ok"
