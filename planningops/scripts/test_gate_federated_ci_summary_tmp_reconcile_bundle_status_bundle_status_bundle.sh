...
cat >"$REPORT_PATH" <<'JSON'
{
  ...
  "run_id": "reconcile-bundle-status-bundle-status-bundle-gate-sample",
...
cat >"$VALIDATION_PATH" <<'JSON'
{
  ...
  "reconcile_run_id": "reconcile-bundle-status-bundle-status-bundle-gate-sample",
...
output="$("$ROOT_DIR/scripts/gate_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle.sh" \
...
printf '%s\n' "$output" | rg -q "bundle path: ${STATUS_BUNDLE_STATUS_BUNDLE_REAL}"
printf '%s\n' "$output" | rg -q "bundle validation output path: ${STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_REAL}"
...
printf '%s\n' "$output" | rg -q "run_id: reconcile-bundle-status-bundle-status-bundle-gate-sample"
...
python3 - <<'PY' "$STATUS_BUNDLE_STATUS_BUNDLE_PATH"
import json
import sys
from pathlib import Path
...
if [[ "$rc" -eq 0 ]]; then
  echo "expected tmp-summary reconcile bundle status-bundle-status-bundle gate to fail on stale bundle validation state"
  exit 1
fi
...
echo "gate federated ci summary tmp reconcile bundle status bundle status bundle ok"
