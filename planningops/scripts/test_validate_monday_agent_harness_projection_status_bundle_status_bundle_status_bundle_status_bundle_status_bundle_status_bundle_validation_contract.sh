...
SCHEMA_PATH="${ROOT_DIR}/planningops/schemas/monday-agent-harness-projection-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-validation.schema.json"
...
python3 "${ROOT_DIR}/planningops/scripts/validate_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle.py" \
  --bundle-file "${FINAL_BUNDLE}" \
  --output "${FINAL_BUNDLE_VALIDATION}" \
  --strict >/dev/null

python3 - <<'PY' "${ROOT_DIR}" "${FINAL_BUNDLE_VALIDATION}" "${SCHEMA_PATH}" "${FINAL_BUNDLE}" "${NEXT_STATUS}" "${NEXT_STATUS_VALIDATION}" "${NEXT_BUNDLE}"
import json
import sys
...
assert report["status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path"] == str(bundle_path.resolve()), report
...
assert report["bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_path"] == str(status_validation_path.resolve()), report
...
assert report["resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path"] == str(bundle_path.resolve()), report
PY
...
doc["resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path"] = str(
...
doc["bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_report"]["bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_output_path"] = str(
...
if [[ "${rc}" -eq 0 ]]; then
  echo "expected strict failure for invalid monday harness projection status bundle status bundle status bundle status bundle status bundle"
  exit 1
fi
...
assert any("resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path must match the validated bundle file path" in item for item in report["errors"]), report
...
assert any("bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_report.bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_output_path must match bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_path" in item for item in report["errors"]), report
assert any("bundle must match canonical monday harness projection status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status bundle resolution" in item for item in report["errors"]), report
PY

echo "validate monday agent harness projection status bundle status bundle status bundle status bundle status bundle validation contract ok"
