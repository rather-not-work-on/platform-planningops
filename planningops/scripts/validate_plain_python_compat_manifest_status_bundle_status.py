...
DEFAULT_SCHEMA = WORKSPACE_ROOT / "planningops/schemas/plain-python-compat-manifest-status-bundle-status.schema.json"
DEFAULT_OUTPUT = (
...
def validate_semantics(status_path: Path, doc: dict[str, Any]) -> tuple[list[str], list[str], dict[str, Any] | None]:
    ...
    status_sidecar_path = doc.get("status_path")
    status_validation_sidecar_path = doc.get("status_validation_path")
    ...
    for key, value in (
        ...
        ("status_path", status_sidecar_path),
        ("status_validation_path", status_validation_sidecar_path),
    ):
    ...
    if isinstance(bundle_path, str) and isinstance(resolved_status_bundle_path, str) and bundle_path != resolved_status_bundle_path:
    ...
    if bundle_status_verdict != "pass" or bundle_status_validation_verdict != "pass" or bundle_validation_verdict != "pass":
        expected_verdict = "fail"
    if verdict != expected_verdict:
        append_unique(errors, "verdict must match bundle status/bundle validation verdict aggregation")
    ...
    if isinstance(validation_doc, dict):
        ...
        if resolved_status_bundle_path != validation_doc.get("resolved_status_bundle_path"):
            append_unique(errors, "resolved_status_bundle_path must match bundle-validation report resolved_status_bundle_path")
        ...
        if status_sidecar_path != validation_doc.get("status_path"):
            append_unique(errors, "status_path must match bundle-validation report status_path")
        if status_validation_sidecar_path != validation_doc.get("status_validation_path"):
...
def build_validation_report(status_path: Path, schema_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    semantic_errors, warnings, semantic_report = validate_semantics(status_path, doc)
    ...
    if doc.get("bundle_status_validation_output_path") != str(output_path.resolve()):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate plain python compatibility manifest status-bundle status report.")
