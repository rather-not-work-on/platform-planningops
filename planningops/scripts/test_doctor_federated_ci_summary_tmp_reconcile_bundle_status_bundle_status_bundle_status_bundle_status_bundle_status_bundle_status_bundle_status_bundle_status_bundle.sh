...
REPORT_OUTPUT="$TMP_DIR/doctor-status-bundle-status-bundle-status-bundle-status-bundle-status.txt"
BROKEN_REPORT_OUTPUT="$TMP_DIR/doctor-status-bundle-status-bundle-status-bundle-status-bundle-status-broken.txt"
...
cat >"$REPORT_PATH" <<'JSON'
{
  ...
  "run_id": "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-doctor-sample",
...
cat >"$VALIDATION_PATH" <<'JSON'
{
  ...
  "reconcile_run_id": "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-doctor-sample",
...
grep -q "run_id: reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-doctor-sample" "$REPORT_OUTPUT"
...
assert bundle["run_id"] == "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-doctor-sample", bundle
...
assert canonical(validation["bundle_path"]) == canonical(sys.argv[1]), validation
assert validation["run_id"] == "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-doctor-sample", validation
...
assert canonical(status["output_path"]) == canonical(sys.argv[3]), status
assert canonical(status["status_validation_output_path"]) == canonical(sys.argv[4]), status
assert status["run_id"] == "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-doctor-sample", status
...
assert canonical(status_validation["status_path"]) == canonical(sys.argv[3]), status_validation
assert canonical(status_validation["output_path"]) == canonical(sys.argv[4]), status_validation
assert canonical(status_validation["status_output_path"]) == canonical(sys.argv[3]), status_validation
assert canonical(status_validation["status_validation_output_path"]) == canonical(sys.argv[4]), status_validation
assert status_validation["status_run_id"] == "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-doctor-sample", status_validation
...
if python3 "$ROOT_DIR/scripts/doctor_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle.py" \
  --bundle-file "$BROKEN_BUNDLE_PATH" \
  --bundle-validation-output "$NEXT_BUNDLE_VALIDATION_PATH" \
  --status-output "$NEXT_DOCTOR_STATUS_PATH" \
  --status-validation-output "$NEXT_DOCTOR_STATUS_VALIDATION_PATH" \
  --require-pass >"$BROKEN_REPORT_OUTPUT" 2>&1; then
  echo "expected tmp-summary reconcile bundle status-bundle-status-bundle-status-bundle-status-bundle-status doctor to fail on broken bundle"
  exit 1
fi
...
echo "doctor federated ci summary tmp reconcile bundle status bundle status bundle status bundle status bundle status bundle status ok"
