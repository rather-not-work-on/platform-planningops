...
import resolve_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status as previous_layer
...
STATUS_VALIDATION_STATUS_PATH_FIELD = STATUS_PATH_FIELD
STATUS_OUTPUT_FIELD = "bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_output_path"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status + validation pair."
        )
    )
...
def ensure_status_matches_validation(
    *,
    ...
    if (
        resolve_doc_path(status_doc.get(STATUS_VALIDATION_OUTPUT_FIELD), base=status_path.parent)
    ...
    if (
        resolve_doc_path(
            status_validation_doc.get(STATUS_VALIDATION_OUTPUT_FIELD),
            base=status_validation_path.parent,
        )
        != status_validation_path
    ):
        raise SystemExit(
            f"status-validation report {STATUS_VALIDATION_OUTPUT_FIELD} must match the resolved validation path"
        )
...
def resolve_from_status(
    ...
    *,
    ...
    validate_doc_schema(
        status_doc,
        status_schema_path,
        "monday harness projection status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status report",
    )
    ...
    validate_doc_schema(
        ...
        "monday harness projection status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-validation report",
    )
...
def resolve_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle(
    *,
    ...
    if status_file:
        ...
        if is_status_doc(artifact_doc):
            ...
            raise SystemExit(
                ...
                "status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status report or validation report"
            )
    ...
    validate_doc_schema(
        bundle,
        bundle_schema_path,
        "monday harness projection status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status bundle",
    )
...
def main() -> int:
    args = parse_args()
    bundle = resolve_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle(
