...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Resolve the canonical plain-python manifest status-bundle-status-bundle-status-bundle-status-bundle-status + validation sidecar pair."
    )
...
def build_bundle(
    *,
    ...
    return {
        ...
        "bundle_status_bundle_status_bundle_status_bundle_validation_path": str(status_validation_path.resolve()),
...
def resolve_from_status(
    ...
    *,
    ...
    validate_doc_schema(
        status_doc,
        status_schema_path,
        "plain python manifest status-bundle-status-bundle-status-bundle-status-bundle-status report",
    )
    ...
    if embedded_status_path != status_path:
        raise SystemExit(
            "bundle_status_bundle_status_bundle_status_bundle_output_path must match the resolved status-bundle-status-bundle-status-bundle-status-bundle-status report path"
        )
    ...
    if status_validation_path is None:
        raise SystemExit("missing bundle_status_bundle_status_bundle_status_bundle_validation_output_path")
    ...
    validate_doc_schema(
        ...
        "plain python manifest status-bundle-status-bundle-status-bundle-status-bundle-status-validation report",
    )
    ...
    if (
        ...
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status-bundle-status-validation report bundle_status_bundle_status_bundle_status_bundle_status_path must match the resolved status report"
        )
    if (
        resolve_doc_path(
        ...
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status-bundle-status-validation report bundle_status_bundle_status_bundle_status_bundle_output_path must match the resolved status report"
        )
    if resolve_doc_path(status_validation_doc.get("output_path"), base=status_validation_path.parent) != status_validation_path:
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status-bundle-status-validation report output_path must match the resolved validation report path"
        )
    if (
        resolve_doc_path(
        ...
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status-bundle-status-validation report bundle_status_bundle_status_bundle_status_bundle_validation_output_path must match the resolved validation report path"
        )
...
def resolve_from_status_validation(
    ...
    *,
    ...
    if status_path is None:
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status-bundle-status-validation report missing bundle_status_bundle_status_bundle_status_bundle_output_path"
        )
    if (
        resolve_doc_path(
            status_validation_doc.get("bundle_status_bundle_status_bundle_status_bundle_status_path"),
        ...
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status-bundle-status-validation report bundle_status_bundle_status_bundle_status_bundle_status_path must match bundle_status_bundle_status_bundle_status_bundle_output_path"
        )
    ...
    if (
        ...
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status-bundle-status report bundle_status_bundle_status_bundle_status_bundle_output_path must match the resolved status report"
        )
    if (
        resolve_doc_path(
            status_doc.get("bundle_status_bundle_status_bundle_status_bundle_validation_output_path"),
        ...
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status-bundle-status report bundle_status_bundle_status_bundle_status_bundle_validation_output_path must match the resolved validation report"
        )
    ...
    *,
    ...
    if status_file:
        ...
        if is_status_doc(artifact_doc):
            ...
            raise SystemExit(
                "artifact is neither a status-bundle-status-bundle-status-bundle-status-bundle-status report nor a status-bundle-status-bundle-status-bundle-status-bundle-status-validation report"
            )

    output_path = resolve_output_path(output)
    if output_path is not None:
        bundle = dict(bundle)
        bundle["resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path"] = str(output_path)
    validate_doc_schema(
        bundle,
        bundle_schema_path,
        "plain python manifest status-bundle-status-bundle-status-bundle-status-bundle-status bundle",
    )
