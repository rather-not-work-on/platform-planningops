...
import validate_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle as bundle_validation
import validate_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status as status_validation
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Print plain python compatibility manifest "
...
def resolve_bundle(args: argparse.Namespace) -> tuple[Path, dict]:
    ...
    bundle_doc = bundle_resolver.resolve_status_bundle_status_bundle_status_bundle_status_bundle_status(
...
def build_status_report(
    ...
    if bundle_doc.get("bundle_validation_verdict") != "pass" and "bundle validation verdict is fail" not in errors:
        errors.append("bundle validation verdict is fail")
    if validation_report.get("verdict") != "pass" and "fresh bundle validation verdict is fail" not in errors:
        errors.append("fresh bundle validation verdict is fail")
    ...
    if not ready:
        next_step = (
            "python3 planningops/scripts/"
            "doctor_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_"
            "status_bundle_status_bundle_status_bundle.py --bundle-file "
            f"{bundle_path} --require-pass"
        )

    return {
        "bundle_path": str(bundle_path.resolve()),
        "resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path": str(
            status_output.resolve()
        ),
        "bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_output_path": str(
            status_output.resolve()
        ),
        "bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_output_path": str(
            status_validation_output.resolve()
        ),
        "bundle_validation_output_path": validation_report.get("output_path"),
        "bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_path": bundle_doc.get(
            "bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_path"
        ),
        "bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_path": bundle_doc.get(
...
def render_lines(kind: str, bundle_path: Path, bundle_doc: dict, validation_report: dict, status_report: dict) -> list[str]:
    ...
    return [
        ...
        f"bundle status output path: {status_report.get('bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_output_path') or 'missing'}",
        f"bundle status validation output path: {status_report.get('bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_output_path') or 'missing'}",
        f"bundle validation output path: {validation_report.get('output_path') or 'missing'}",
        f"bundle status bundle status bundle status bundle status path: {bundle_doc.get('bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_path') or 'missing'}",
        f"bundle status bundle status bundle status bundle validation path: {bundle_doc.get('bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_path') or 'missing'}",
