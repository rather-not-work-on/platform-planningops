...
def validate_semantics(status_path: Path, doc: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    if bundle_status_output_path != str(status_path.resolve()):
        append_unique(
            errors,
            "bundle_status_bundle_status_bundle_output_path must match the validated status file path",
        )
    if not isinstance(bundle_status_validation_output_path, str) or not bundle_status_validation_output_path.strip():
        append_unique(
            errors,
            "bundle_status_bundle_status_bundle_validation_output_path must be a non-empty string",
        )
    ...
    if (
        ...
        append_unique(errors, "resolved_status_bundle_status_bundle_status_bundle_path must match bundle_path")
    ...
    if verdict != expected_verdict:
        append_unique(
            errors,
            "verdict must match propagated monday projection status-bundle-status-bundle-status-bundle aggregation",
        )
    ...
    if isinstance(bundle_path, Path):
        if not bundle_path.exists():
            append_unique(
                errors,
                "bundle_path must point at an existing resolved status-bundle-status-bundle-status-bundle artifact",
            )
        else:
    ...
    if isinstance(validation_doc, dict):
        ...
        if bundle_path is not None and validation_doc.get("status_bundle_status_bundle_status_bundle_path") != str(
            bundle_path.resolve()
        ):
            append_unique(
                errors,
                "bundle_path must match bundle-validation report status_bundle_status_bundle_status_bundle_path",
            )
        ...
        if resolved_bundle_path != validation_doc.get("resolved_status_bundle_status_bundle_status_bundle_path"):
            append_unique(
                errors,
                "resolved_status_bundle_status_bundle_status_bundle_path must match bundle-validation report resolved_status_bundle_status_bundle_status_bundle_path",
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate monday agent harness projection status-bundle-status-bundle-status-bundle status report."
    )
