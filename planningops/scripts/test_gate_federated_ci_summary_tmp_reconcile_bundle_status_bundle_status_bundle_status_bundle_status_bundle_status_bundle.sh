...
cat >"$REPORT_PATH" <<'JSON'
{
  ...
  "run_id": "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-gate-sample",
...
cat >"$VALIDATION_PATH" <<'JSON'
{
  ...
  "reconcile_run_id": "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-gate-sample",
...
python3 "$ROOT_DIR/scripts/validate_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle.py" --bundle-file "$NEXT_BUNDLE_PATH" --output "$NEXT_BUNDLE_VALIDATION_PATH" --strict >/dev/null

output="$("$ROOT_DIR/scripts/gate_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle.sh" \
...
printf '%s\n' "$output" | rg -q "bundle path: ${NEXT_BUNDLE_REAL}"
printf '%s\n' "$output" | rg -q "bundle validation output path: ${NEXT_BUNDLE_VALIDATION_REAL}"
...
printf '%s\n' "$output" | rg -q "run_id: reconcile-bundle-status-bundle-status-bundle-status-bundle-status-bundle-gate-sample"
...
set +e
output="$("$ROOT_DIR/scripts/gate_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle.sh" \
  --bundle-file "$NEXT_BUNDLE_PATH" \
  --bundle-validation-output "$NEXT_BUNDLE_VALIDATION_PATH" \
  --status-output "$NEXT_DOCTOR_STATUS_PATH" \
  --status-validation-output "$NEXT_DOCTOR_STATUS_VALIDATION_PATH" 2>&1)"
rc=$?
set -e

if [[ "$rc" -eq 0 ]]; then
  echo "expected tmp-summary reconcile bundle status-bundle-status-bundle-status-bundle-status-bundle gate to fail on stale bundle validation state"
  exit 1
fi
...
echo "gate federated ci summary tmp reconcile bundle status bundle status bundle status bundle status bundle ok"
