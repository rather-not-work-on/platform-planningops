...
import validate_plain_python_compat_manifest_status_bundle as bundle_validation
import validate_plain_python_compat_manifest_status_bundle_status as status_validation
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Print plain python compatibility manifest status bundle status")
...
def resolve_bundle(args: argparse.Namespace) -> tuple[Path, dict]:
    if args.bundle_file:
        ...
        bundle_doc = bundle_resolver.load_json(bundle_path)
        return bundle_path, bundle_doc

    bundle_doc = bundle_resolver.resolve_status_bundle(
...
def build_status_report(
    ...
    if bundle_doc.get("status_verdict") != "pass" and "bundle status verdict is fail" not in errors:
        errors.append("bundle status verdict is fail")
    if bundle_doc.get("status_validation_verdict") != "pass" and "bundle status validation verdict is fail" not in errors:
    ...
    if not ready:
        next_step = (
            "python3 planningops/scripts/doctor_plain_python_compat_manifest_status_bundle.py --bundle-file "
            f"{bundle_path} --require-pass"
        )

    return {
        "bundle_path": str(bundle_path.resolve()),
        "resolved_status_bundle_path": bundle_doc.get("resolved_status_bundle_path"),
        "bundle_status_output_path": str(status_output.resolve()),
        "bundle_status_validation_output_path": str(status_validation_output.resolve()),
        ...
        "resolved_guardrail_script_paths": bundle_doc.get("resolved_guardrail_script_paths"),
        "status_path": bundle_doc.get("status_path"),
        "status_validation_path": bundle_doc.get("status_validation_path"),
        "bundle_status_verdict": bundle_doc.get("status_verdict"),
        "bundle_status_validation_verdict": bundle_doc.get("status_validation_verdict"),
...
def render_lines(kind: str, bundle_path: Path, bundle_doc: dict, validation_report: dict) -> list[str]:
    ...
    return [
        ...
        f"bundle status output path: {bundle_doc.get('bundle_status_output_path') or 'missing'}",
        f"bundle status validation output path: {bundle_doc.get('bundle_status_validation_output_path') or 'missing'}",
        f"bundle validation output path: {validation_report.get('output_path') or 'missing'}",
        f"bundle verdict: {bundle_doc.get('status_verdict') or 'unknown'}",
        f"bundle status validation verdict: {bundle_doc.get('status_validation_verdict') or 'unknown'}",
...
def main() -> int:
    ...
    bundle_validation.manifest_validation.write_json(Path(args.bundle_validation_output), validation_report)
    ...
    bundle_validation.manifest_validation.write_json(Path(args.status_output), status_report)
    status_validation_report = status_validation.build_validation_report(
        Path(args.status_output),
        status_validation.DEFAULT_SCHEMA,
        Path(args.status_validation_output),
    )
    bundle_validation.manifest_validation.write_json(Path(args.status_validation_output), status_validation_report)
    rendered_bundle_doc = dict(bundle_doc)
    rendered_bundle_doc["bundle_status_output_path"] = status_report["bundle_status_output_path"]
    rendered_bundle_doc["bundle_status_validation_output_path"] = status_report["bundle_status_validation_output_path"]
    for line in render_lines(kind, bundle_path, rendered_bundle_doc, validation_report):
        print(line)
