...
def validate_semantics(status_path: Path, doc: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    if (
        ...
        append_unique(
            errors,
            "resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path must match bundle_path",
        )
    ...
    if verdict != expected_verdict:
        append_unique(
            errors,
            "verdict must match propagated monday projection status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle aggregation",
        )
    ...
    if isinstance(bundle_path, Path):
        if not bundle_path.exists():
            append_unique(
                errors,
                "bundle_path must point at an existing resolved status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle artifact",
            )
        else:
    ...
    if isinstance(validation_doc, dict):
        ...
        if bundle_path is not None and validation_doc.get(
            "status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path"
        ) != str(bundle_path.resolve()):
            append_unique(
                errors,
                "bundle_path must match bundle-validation report status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path",
            )
        ...
        if resolved_bundle_path != validation_doc.get(
            ...
            append_unique(
                errors,
                "resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path must match bundle-validation report resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path",
            )
...
def build_validation_report(status_path: Path, schema_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    return {
        "bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_path": str(status_path.resolve()),
