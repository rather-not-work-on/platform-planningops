...
STATUS_BUNDLE_FROM_VALIDATION_PATH="${TMP_DIR}/plain-python-compat-status-bundle-from-validation.json"
BUNDLE_SCHEMA_PATH="${ROOT_DIR}/planningops/schemas/plain-python-compat-manifest-status-bundle.schema.json"
BROKEN_STATUS_PATH="${TMP_DIR}/broken-status.json"
BROKEN_STATUS_VALIDATION_PATH="${TMP_DIR}/broken-status-validation.json"
...
python3 "${ROOT_DIR}/planningops/scripts/resolve_plain_python_compat_manifest_status.py" \
  --artifact-file "${STATUS_PATH}" \
  --output "${STATUS_BUNDLE_PATH}" >/dev/null

python3 "${ROOT_DIR}/planningops/scripts/resolve_plain_python_compat_manifest_status.py" \
  --status-validation-file "${STATUS_VALIDATION_PATH}" \
  --output "${STATUS_BUNDLE_FROM_VALIDATION_PATH}" >/dev/null

python3 - <<'PY' "${ROOT_DIR}" "${STATUS_BUNDLE_PATH}" "${STATUS_BUNDLE_FROM_VALIDATION_PATH}" "${BUNDLE_SCHEMA_PATH}" "${STATUS_PATH}" "${STATUS_VALIDATION_PATH}"
import json
import sys
...
assert bundle["resolved_guardrail_script_paths"] == bundle["status_report"]["resolved_guardrail_script_paths"], bundle
assert bundle["resolved_guardrail_script_paths"] == bundle["status_validation_report"]["resolved_guardrail_script_paths"], bundle
...
assert bundle["status_report"]["status_validation_output_path"] == str(status_validation_path.resolve()), bundle
assert bundle["status_validation_report"]["status_output_path"] == str(status_path.resolve()), bundle
...
python3 - <<'PY' "${STATUS_PATH}" "${BROKEN_STATUS_PATH}"
import json
import sys
from pathlib import Path
...
if python3 "${ROOT_DIR}/planningops/scripts/resolve_plain_python_compat_manifest_status.py" \
  --artifact-file "${BROKEN_STATUS_PATH}" >/dev/null 2>&1; then
  echo "expected status resolver to fail on mismatched status_validation_output_path" >&2
  exit 1
fi

python3 - <<'PY' "${STATUS_VALIDATION_PATH}" "${BROKEN_STATUS_VALIDATION_PATH}"
import json
import sys
from pathlib import Path
...
if python3 "${ROOT_DIR}/planningops/scripts/resolve_plain_python_compat_manifest_status.py" \
  ...
  echo "expected status resolver to fail on mismatched status_output_path" >&2
  exit 1
fi
...
if python3 "${ROOT_DIR}/planningops/scripts/resolve_plain_python_compat_manifest_status.py" \
  --status-validation-file "${BROKEN_STATUS_VALIDATION_PATH}" >/dev/null 2>&1; then
  echo "expected status resolver to fail on mismatched resolved_guardrail_script_paths" >&2
  exit 1
fi

echo "resolve plain python compat manifest status ok"
