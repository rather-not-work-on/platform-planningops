...
SCHEMA_PATH="${ROOT_DIR}/planningops/schemas/monday-agent-harness-projection-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status.schema.json"
...
python3 - <<'PY' "${ROOT_DIR}" "${NEW_STATUS}" "${NEW_STATUS_VALIDATION}" "${SCHEMA_PATH}" "${FINAL_BUNDLE}" "${FINAL_BUNDLE_VALIDATION}"
import json
import sys
...
if [[ "${rc}" -eq 0 ]]; then
  echo "expected strict failure for invalid monday harness projection status bundle status bundle status bundle status bundle status bundle status bundle status" >&2
  exit 1
fi
...
assert any("resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path must match bundle_path" in item for item in report["errors"]), report
assert any("ready must be false when verdict=fail" in item or "verdict must match propagated monday projection status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle aggregation" in item for item in report["errors"]), report
PY

echo "validate monday agent harness projection status bundle status bundle status bundle status bundle status bundle status bundle status contract ok"
