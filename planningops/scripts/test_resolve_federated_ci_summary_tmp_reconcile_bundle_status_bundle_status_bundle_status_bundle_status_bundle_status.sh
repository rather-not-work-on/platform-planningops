...
if python3 "$ROOT_DIR/scripts/resolve_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --artifact-file "$NEXT_DOCTOR_STATUS_VALIDATION_PATH" \
  --output "$TMP_DIR/broken-bundle.json" >"$TMP_DIR/broken.stdout" 2>"$BROKEN_STDERR"; then
  echo "expected stale tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation resolution to fail" >&2
  exit 1
fi

grep -q "tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status-validation report must match status run_id" "$BROKEN_STDERR"
