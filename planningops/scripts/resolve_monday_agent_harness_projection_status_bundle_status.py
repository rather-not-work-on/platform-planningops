...
DEFAULT_STATUS_VALIDATION_SCHEMA = (
    WORKSPACE_ROOT / "planningops/schemas/monday-agent-harness-projection-status-bundle-status-validation.schema.json"
)
DEFAULT_BUNDLE_SCHEMA = (
    WORKSPACE_ROOT / "planningops/schemas/monday-agent-harness-projection-status-bundle-status-bundle.schema.json"
)
DEFAULT_OUTPUT = (
    WORKSPACE_ROOT / "planningops/artifacts/validation/monday-agent-harness-projection-status-bundle-status-bundle.json"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Resolve the canonical monday harness projection status-bundle-status + validation pair."
    )
...
def resolve_from_status(
    ...
    *,
    ...
    validate_doc_schema(status_doc, status_schema_path, "monday harness projection status-bundle-status report")

    if resolve_doc_path(status_doc.get("bundle_status_output_path"), base=status_path.parent) != status_path:
        raise SystemExit("status-bundle-status report bundle_status_output_path must match the resolved status path")

    status_validation_path = resolve_doc_path(status_doc.get("bundle_status_validation_output_path"), base=status_path.parent)
    if status_validation_path is None:
        raise SystemExit("status-bundle-status report missing bundle_status_validation_output_path")
    ...
    validate_doc_schema(
        ...
        "monday harness projection status-bundle-status-validation report",
    )

    if resolve_doc_path(status_validation_doc.get("bundle_status_path"), base=status_validation_path.parent) != status_path:
        raise SystemExit("status-bundle-status-validation report bundle_status_path must match the resolved status path")
    if resolve_doc_path(status_validation_doc.get("bundle_status_output_path"), base=status_validation_path.parent) != status_path:
        raise SystemExit(
            "status-bundle-status-validation report bundle_status_output_path must match the resolved status path"
        )
    if resolve_doc_path(status_validation_doc.get("output_path"), base=status_validation_path.parent) != status_validation_path:
        raise SystemExit(
            "status-bundle-status-validation report output_path must match the resolved validation path"
        )
    if (
        resolve_doc_path(status_validation_doc.get("bundle_status_validation_output_path"), base=status_validation_path.parent)
        != status_validation_path
    ):
        raise SystemExit(
            "status-bundle-status-validation report bundle_status_validation_output_path must match the resolved validation path"
        )
    for status_field, validation_field in (
        ...
        if status_doc.get(status_field) != status_validation_doc.get(validation_field):
            raise SystemExit(
                f"status-bundle-status-validation report {validation_field} must match status {status_field}"
            )
...
def resolve_from_status_validation(
    ...
    *,
    ...
    if resolve_doc_path(status_validation_doc.get("output_path"), base=status_validation_path.parent) != status_validation_path:
        raise SystemExit("status-bundle-status-validation report output_path must match the resolved validation path")
    ...
    if (
        ...
        raise SystemExit(
            "status-bundle-status report bundle_status_validation_output_path must match the resolved validation path"
        )
    for status_field, validation_field in (
        ("bundle_path", "bundle_path"),
        ("resolved_status_bundle_path", "resolved_status_bundle_path"),
    ...
    *,
    ...
    if status_file:
        ...
        if is_status_doc(artifact_doc):
            ...
            raise SystemExit(
                "artifact must be a monday harness projection status-bundle-status report or validation report"
            )

    output_path = resolve_output_path(output)
    if output_path is not None:
        bundle = dict(bundle)
        bundle["resolved_status_bundle_status_bundle_path"] = str(output_path.resolve())
    validate_doc_schema(bundle, bundle_schema_path, "monday harness projection status-bundle-status bundle")
