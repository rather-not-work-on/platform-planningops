...
BUNDLE_SCHEMA="${ROOT_DIR}/planningops/schemas/monday-agent-harness-projection-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle.schema.json"
...
python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --status-validation-file "${CURRENT_STATUS_VALIDATION}" \
  --output "${FINAL_BUNDLE_FROM_VALIDATION}" >/dev/null

python3 - <<'PY' "${ROOT_DIR}" "${FINAL_BUNDLE}" "${FINAL_BUNDLE_FROM_VALIDATION}" "${BUNDLE_SCHEMA}" "${CURRENT_STATUS}" "${CURRENT_STATUS_VALIDATION}" "${CURRENT_BUNDLE}"
import json
import sys
...
assert bundle["bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_path"] == str(status_path.resolve()), bundle
assert bundle["bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_path"] == str(status_validation_path.resolve()), bundle
assert bundle["bundle_path"] == str(outer_bundle_path.resolve()), bundle
assert bundle["resolved_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_path"] == str(outer_bundle_path.resolve()), bundle
...
assert bundle["bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_report"]["bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_output_path"] == str(status_validation_path.resolve()), bundle
assert bundle["bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_report"]["bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_output_path"] == str(status_path.resolve()), bundle
...
python3 - <<'PY' "${CURRENT_STATUS}" "${BROKEN_STATUS}"
import json
import sys
from pathlib import Path
...
if python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --artifact-file "${BROKEN_STATUS}" >/dev/null 2>&1; then
  echo "expected monday projection next outer status-bundle resolver to fail on mismatched validation output path" >&2
  exit 1
fi

python3 - <<'PY' "${CURRENT_STATUS_VALIDATION}" "${BROKEN_STATUS_VALIDATION}"
import json
import sys
from pathlib import Path
...
if python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  ...
  echo "expected monday projection next outer status-bundle resolver to fail on mismatched status output path" >&2
  exit 1
fi

echo "resolve monday agent harness projection status bundle status bundle status bundle status bundle status bundle status bundle status ok"
