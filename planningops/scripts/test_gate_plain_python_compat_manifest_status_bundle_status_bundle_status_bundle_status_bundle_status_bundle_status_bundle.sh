...
bash "${ROOT_DIR}/planningops/scripts/gate_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle.sh" \
...
if bash "${ROOT_DIR}/planningops/scripts/gate_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle.sh" \
  --bundle-file "${BROKEN_BUNDLE}" \
  --status-output "${STATUS6}" \
  --status-validation-output "${STATUS6_VALIDATION}" \
  --bundle-validation-output "${BUNDLE6_VALIDATION}" >/dev/null 2>&1; then
  echo "expected status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle gate to fail on broken bundle" >&2
  exit 1
fi

echo "gate plain python compat manifest status bundle status bundle status bundle status bundle status bundle status bundle ok"
