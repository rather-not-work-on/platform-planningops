...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "status-bundle-status-bundle-status-bundle-status + validation pair."
        )
    )
...
def resolve_from_status(
    ...
    *,
    ...
    validate_doc_schema(
        status_doc,
        status_schema_path,
        "monday harness projection status-bundle-status-bundle-status-bundle-status report",
    )
    ...
    if (
        ...
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status report "
            "bundle_status_bundle_status_bundle_output_path must match the resolved status path"
        )
    ...
    if status_validation_path is None:
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status report missing "
            "bundle_status_bundle_status_bundle_validation_output_path"
        )
    ...
    validate_doc_schema(
        ...
        "monday harness projection status-bundle-status-bundle-status-bundle-status-validation report",
    )
    ...
    if (
        ...
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status-validation report "
            "bundle_status_bundle_status_bundle_status_path must match the resolved status path"
        )
    if (
        resolve_doc_path(
    ...
    if (
        ...
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status-validation report "
            "bundle_status_bundle_status_bundle_validation_output_path must match the resolved validation path"
        )
...
def resolve_from_status_validation(
    ...
    *,
    ...
    if status_path is None:
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status-validation report missing "
            "bundle_status_bundle_status_bundle_output_path"
        )
    if (
        resolve_doc_path(
        ...
        raise SystemExit(
            "status-bundle-status-bundle-status-bundle-status-validation report "
            "bundle_status_bundle_status_bundle_status_path must match "
            "bundle_status_bundle_status_bundle_output_path"
        )
    ...
    if status_file:
        ...
        if is_status_doc(artifact_doc):
            ...
            raise SystemExit(
                ...
                "status-bundle-status-bundle-status-bundle-status report or validation report"
            )

    output_path = resolve_output_path(output)
    if output_path is not None:
        bundle = dict(bundle)
        bundle["resolved_status_bundle_status_bundle_status_bundle_status_bundle_path"] = str(output_path.resolve())
    validate_doc_schema(
        bundle,
        bundle_schema_path,
        "monday harness projection status-bundle-status-bundle-status-bundle-status bundle",
    )
