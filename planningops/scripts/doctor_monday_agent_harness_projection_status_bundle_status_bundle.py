...
import validate_monday_agent_harness_projection_status_bundle_status_bundle as bundle_validation
import validate_monday_agent_harness_projection_status_bundle_status_bundle_status as status_validation
...
BASE_DOCTOR = (
    bundle_resolver.WORKSPACE_ROOT / "planningops/scripts/doctor_monday_agent_harness_projection_status_bundle.py"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Print monday agent harness projection resolved status-bundle-status-bundle health"
    )
...
def doctor_errors(bundle_doc: dict, validation_report: dict) -> list[str]:
    ...
    if bundle_doc.get("ready") is not True:
        append_unique(errors, "resolved status bundle status bundle ready is false")
    ...
    if bundle_doc.get("next_step") != "none":
        append_unique(errors, "resolved status bundle status bundle next_step is not none")
    return errors
...
def build_status_report(
    ...
    *,
    ...
    ready = verdict == "pass"
    next_step = (
        "none" if ready else (bundle_doc.get("next_step") or "inspect monday projection status bundle status bundle")
    )
    return {
        "bundle_path": str(bundle_path.resolve()),
        "resolved_status_bundle_status_bundle_path": bundle_doc.get("resolved_status_bundle_status_bundle_path")
...
def render_lines(kind: str, bundle_path: Path, bundle_doc: dict, validation_report: dict) -> list[str]:
    ...
    next_step = (
        "none" if doctor_verdict == "pass" else (bundle_doc.get("next_step") or "inspect monday projection status bundle status bundle")
    )
    return [
        f"source kind: {kind}",
