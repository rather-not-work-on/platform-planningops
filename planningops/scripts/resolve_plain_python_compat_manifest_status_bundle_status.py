...
DEFAULT_STATUS = WORKSPACE_ROOT / "planningops/artifacts/validation/plain-python-compat-manifest-status-bundle-status.json"
DEFAULT_STATUS_VALIDATION = (
    WORKSPACE_ROOT / "planningops/artifacts/validation/plain-python-compat-manifest-status-bundle-status-validation.json"
)
DEFAULT_STATUS_SCHEMA = WORKSPACE_ROOT / "planningops/schemas/plain-python-compat-manifest-status-bundle-status.schema.json"
DEFAULT_STATUS_VALIDATION_SCHEMA = (
    WORKSPACE_ROOT / "planningops/schemas/plain-python-compat-manifest-status-bundle-status-validation.schema.json"
)
DEFAULT_BUNDLE_SCHEMA = (
    WORKSPACE_ROOT / "planningops/schemas/plain-python-compat-manifest-status-bundle-status-bundle.schema.json"
)
DEFAULT_OUTPUT = (
    WORKSPACE_ROOT / "planningops/artifacts/validation/plain-python-compat-manifest-status-bundle-status-bundle.json"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Resolve the canonical plain-python manifest status-bundle-status + validation sidecar pair."
    )
...
def is_status_doc(doc: dict) -> bool:
    return all(
        key in doc
        for key in ("bundle_status_output_path", "bundle_status_validation_output_path", "ready", "next_step")
    )
...
def is_status_validation_doc(doc: dict) -> bool:
    return all(
        ...
        for key in (
            "bundle_status_path",
            "bundle_status_output_path",
            "bundle_status_validation_output_path",
            "output_path",
            "status_ready",
        )
    )
...
def build_bundle(
    *,
    ...
    return {
        ...
        "bundle_status_validation_path": str(status_validation_path.resolve()),
        "bundle_path": status_doc.get("bundle_path"),
        "resolved_status_bundle_path": status_doc.get("resolved_status_bundle_path"),
        ...
        "bundle_status_report": status_doc,
        "bundle_status_validation_report": status_validation_doc,
    }
...
def resolve_from_status(
    ...
    *,
    ...
    validate_doc_schema(status_doc, status_schema_path, "plain python manifest status-bundle-status report")

    embedded_status_path = resolve_doc_path(status_doc.get("bundle_status_output_path"), base=status_path.parent)
    if embedded_status_path != status_path:
        raise SystemExit("bundle_status_output_path must match the resolved status-bundle-status report path")

    status_validation_path = resolve_doc_path(
        status_doc.get("bundle_status_validation_output_path"),
        base=status_path.parent,
    )
    if status_validation_path is None:
        raise SystemExit("missing bundle_status_validation_output_path")
    ...
    validate_doc_schema(
        ...
        "plain python manifest status-bundle-status-validation report",
    )

    if resolve_doc_path(status_validation_doc.get("bundle_status_path"), base=status_validation_path.parent) != status_path:
        raise SystemExit("status-bundle-status-validation report bundle_status_path must match the resolved status report")
    if (
        resolve_doc_path(status_validation_doc.get("bundle_status_output_path"), base=status_validation_path.parent)
        != status_path
    ):
        raise SystemExit(
            "status-bundle-status-validation report bundle_status_output_path must match the resolved status report"
        )
    if resolve_doc_path(status_validation_doc.get("output_path"), base=status_validation_path.parent) != status_validation_path:
        raise SystemExit(
            "status-bundle-status-validation report output_path must match the resolved validation report path"
        )
    if (
        resolve_doc_path(
            status_validation_doc.get("bundle_status_validation_output_path"),
            base=status_validation_path.parent,
        )
        != status_validation_path
    ):
        raise SystemExit(
            "status-bundle-status-validation report bundle_status_validation_output_path must match the resolved validation report path"
        )
    if status_validation_doc.get("resolved_guardrail_script_paths") != status_doc.get("resolved_guardrail_script_paths"):
        raise SystemExit("status-bundle-status-validation report resolved_guardrail_script_paths must match the status report")
...
def resolve_from_status_validation(
    ...
    *,
    ...
    status_path = resolve_doc_path(status_validation_doc.get("bundle_status_output_path"), base=status_validation_path.parent)
    if status_path is None:
        raise SystemExit("status-bundle-status-validation report missing bundle_status_output_path")
    if resolve_doc_path(status_validation_doc.get("bundle_status_path"), base=status_validation_path.parent) != status_path:
        raise SystemExit("status-bundle-status-validation report bundle_status_path must match bundle_status_output_path")

    status_doc = manifest_validation.load_json(status_path)
    validate_doc_schema(status_doc, status_schema_path, "plain python manifest status-bundle-status report")

    if resolve_doc_path(status_doc.get("bundle_status_output_path"), base=status_path.parent) != status_path:
        raise SystemExit("status-bundle-status report bundle_status_output_path must match the resolved status report")
    if (
        resolve_doc_path(status_doc.get("bundle_status_validation_output_path"), base=status_path.parent)
        != status_validation_path
    ):
        raise SystemExit(
            "status-bundle-status report bundle_status_validation_output_path must match the resolved validation report"
        )
    if status_validation_doc.get("resolved_guardrail_script_paths") != status_doc.get("resolved_guardrail_script_paths"):
        raise SystemExit("status-bundle-status-validation report resolved_guardrail_script_paths must match the status report")
...
def resolve_status_bundle_status_bundle(
    *,
    ...
    if status_file:
        ...
        if is_status_doc(artifact_doc):
            ...
            raise SystemExit("artifact is neither a status-bundle-status report nor a status-bundle-status-validation report")

    output_path = resolve_output_path(output)
    if output_path is not None:
        bundle = dict(bundle)
        bundle["resolved_status_bundle_status_bundle_path"] = str(output_path)
    validate_doc_schema(bundle, bundle_schema_path, "plain python manifest status-bundle-status bundle")
...
def main() -> int:
    args = parse_args()
    bundle = resolve_status_bundle_status_bundle(
