...
STATUS_PATH="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status.json"
VALIDATION_PATH="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-validation.json"
SCHEMA_PATH="$ROOT_DIR/schemas/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-validation.schema.json"
...
cat >"$STATUS_PATH" <<'JSON'
{
  ...
  "run_id": "bundle-status-bundle-status-validation-sample",
...
cat >"$VALIDATION_PATH" <<'JSON'
{
  ...
  "status_validation_output_path": "__VALIDATION_PATH__",
  "status_run_id": "bundle-status-bundle-status-validation-sample",
...
if [[ "$rc" -ne 0 ]]; then
  echo "expected broken status-bundle-status validation report to fail schema validation"
  exit 1
fi

echo "validate federated ci summary tmp reconcile bundle status bundle status validation contract ok"
