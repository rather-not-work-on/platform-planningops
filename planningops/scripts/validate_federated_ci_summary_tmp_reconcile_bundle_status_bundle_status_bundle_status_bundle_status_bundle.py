...
from resolve_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status import (
    resolve_status_bundle_status_bundle_status_bundle_status,
)
...
def validate_semantics(
    ...
    if canonical_bundle is not None:
        ...
        if actual != canonical:
            append_unique(
                errors,
                "bundle does not match canonical tmp-summary reconcile bundle status-bundle-status bundle status bundle status resolution",
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate a canonical federated CI tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle"
    )
...
def main() -> int:
    ...
    if args.bundle_file is not None:
        ...
        if isinstance(artifact_file, str) and artifact_file.strip():
            _, canonical_bundle = resolve_status_bundle_status_bundle_status_bundle_status(
        ...
        _, bundle_doc = resolve_status_bundle_status_bundle_status_bundle_status(
