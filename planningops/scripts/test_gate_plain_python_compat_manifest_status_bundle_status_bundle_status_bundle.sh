...
bash "${ROOT_DIR}/planningops/scripts/gate_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle.sh" \
...
python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH}"
import json
import sys
...
if bash "${ROOT_DIR}/planningops/scripts/gate_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle.sh" \
  --bundle-file "${BROKEN_BUNDLE_PATH}" \
  --status-output "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH}" \
  --status-validation-output "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH}" \
  --bundle-validation-output "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH}" >/dev/null 2>&1; then
  echo "expected status-bundle-status-bundle-status-bundle gate to fail on broken bundle" >&2
  exit 1
fi

echo "gate plain python compat manifest status bundle status bundle status bundle ok"
