...
cat >"$REPORT_PATH" <<'JSON'
{
  ...
  "run_id": "reconcile-bundle-status-validation-sample",
...
cat >"$VALIDATION_PATH" <<'JSON'
{
  ...
  "reconcile_run_id": "reconcile-bundle-status-validation-sample",
...
assert report["output_path"] == str(report_path), report
assert report["status_validation_output_path"] == str(report_path), report
assert report["status_run_id"] == "reconcile-bundle-status-validation-sample", report
...
python3 - <<'PY' "$STATUS_VALIDATION_PATH" "$TMP_DIR/invalid-report.json"
import json
import sys
from pathlib import Path
...
doc.pop("status_validation_output_path")
doc.pop("status_output_path")
Path(sys.argv[2]).write_text(json.dumps(doc, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
PY

python3 - <<'PY' "$ROOT_DIR" "$TMP_DIR/invalid-report.json" "$STATUS_VALIDATION_SCHEMA_PATH"
import json
import sys
from pathlib import Path
...
assert any("$.error_count expected type integer" in item for item in errors), errors
assert any("$.status_validation_output_path is required" in item for item in errors), errors
assert any("$.status_output_path is required" in item for item in errors), errors
PY
...
if [[ "$rc" -eq 0 ]]; then
  echo "expected strict failure for invalid tmp-summary reconcile bundle status validation report"
  exit 1
fi
...
echo "validate federated ci summary tmp reconcile bundle status validation contract ok"
