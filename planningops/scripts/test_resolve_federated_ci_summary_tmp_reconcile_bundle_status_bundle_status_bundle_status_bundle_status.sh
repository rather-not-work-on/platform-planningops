...
DOCTOR_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status.json"
DOCTOR_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-status-validation.json"
RESOLVED_FROM_STATUS="$TMP_DIR/resolved-from-status-bundle-status-bundle-status-bundle-status-bundle.json"
RESOLVED_FROM_STATUS_VALIDATION="$TMP_DIR/resolved-from-status-bundle-status-bundle-status-bundle-status-bundle-validation.json"
...
cat >"$REPORT_PATH" <<'JSON'
{
  ...
  "run_id": "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-resolve-sample",
...
cat >"$VALIDATION_PATH" <<'JSON'
{
  ...
  "reconcile_run_id": "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-resolve-sample",
...
python3 "$ROOT_DIR/scripts/doctor_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle.py" \
  ...
  --status-output "$DOCTOR_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH" \
  --status-validation-output "$DOCTOR_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH" \
  --require-pass >/dev/null
...
python3 "$ROOT_DIR/scripts/resolve_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --artifact-file "$DOCTOR_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH" \
  --output "$RESOLVED_FROM_STATUS" >/dev/null

python3 "$ROOT_DIR/scripts/resolve_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --artifact-file "$DOCTOR_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH" \
  --output "$RESOLVED_FROM_STATUS_VALIDATION" >/dev/null

python3 - <<'PY' "$DOCTOR_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH" "$DOCTOR_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH" "$RESOLVED_FROM_STATUS" "$RESOLVED_FROM_STATUS_VALIDATION"
import json
import sys
from pathlib import Path
...
for bundle, artifact_path in (
    ...
    assert bundle["run_id"] == "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-resolve-sample", bundle
...
python3 - <<'PY' "$DOCTOR_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH"
import json
import sys
from pathlib import Path
...
path = Path(sys.argv[1]).resolve()
doc = json.loads(path.read_text(encoding="utf-8"))
doc["status_run_id"] = "reconcile-bundle-status-bundle-status-bundle-status-bundle-status-resolve-sample-stale"
path.write_text(json.dumps(doc, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
PY

if python3 "$ROOT_DIR/scripts/resolve_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --artifact-file "$DOCTOR_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH" \
  --output "$TMP_DIR/broken-bundle.json" >"$TMP_DIR/broken.stdout" 2>"$BROKEN_STDERR"; then
  echo "expected stale tmp-summary reconcile bundle status-bundle-status bundle status bundle status-validation resolution to fail" >&2
  exit 1
fi

grep -q "tmp-summary reconcile bundle status-bundle-status bundle status bundle status-validation report must match status run_id" "$BROKEN_STDERR"

echo "resolve federated ci summary tmp reconcile bundle status bundle status bundle status bundle status ok"
