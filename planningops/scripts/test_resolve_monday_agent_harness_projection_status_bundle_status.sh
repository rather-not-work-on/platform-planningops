...
STATUS_BUNDLE_STATUS_BUNDLE_FROM_VALIDATION="${TMP_DIR}/projection-status-bundle-status-bundle-from-validation.json"
BUNDLE_SCHEMA="${ROOT_DIR}/planningops/schemas/monday-agent-harness-projection-status-bundle-status-bundle.schema.json"
BROKEN_STATUS="${TMP_DIR}/broken-status-bundle-status.json"
BROKEN_STATUS_VALIDATION="${TMP_DIR}/broken-status-bundle-status-validation.json"
...
python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status.py" \
  --status-validation-file "${STATUS_BUNDLE_STATUS_VALIDATION}" \
  --output "${STATUS_BUNDLE_STATUS_BUNDLE_FROM_VALIDATION}" >/dev/null

python3 - <<'PY' "${ROOT_DIR}" "${STATUS_BUNDLE_STATUS_BUNDLE}" "${STATUS_BUNDLE_STATUS_BUNDLE_FROM_VALIDATION}" "${BUNDLE_SCHEMA}" "${STATUS_BUNDLE_STATUS}" "${STATUS_BUNDLE_STATUS_VALIDATION}" "${STATUS_BUNDLE}" "${STATUS_BUNDLE_VALIDATION}"
import json
import sys
...
status_bundle_path = Path(sys.argv[7])
status_bundle_validation_path = Path(sys.argv[8])
...
assert bundle["bundle_path"] == str(status_bundle_path.resolve()), bundle
assert bundle["resolved_status_bundle_path"] == str(status_bundle_path.resolve()), bundle
assert bundle["bundle_validation_output_path"] == str(status_bundle_validation_path.resolve()), bundle
...
python3 - <<'PY' "${STATUS_BUNDLE_STATUS}" "${BROKEN_STATUS}"
import json
import sys
from pathlib import Path
...
if python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status.py" \
  --artifact-file "${BROKEN_STATUS}" >/dev/null 2>&1; then
  echo "expected monday projection status-bundle-status resolver to fail on mismatched bundle_status_validation_output_path" >&2
  exit 1
fi

python3 - <<'PY' "${STATUS_BUNDLE_STATUS_VALIDATION}" "${BROKEN_STATUS_VALIDATION}"
import json
import sys
from pathlib import Path
...
if python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status.py" \
  ...
  echo "expected monday projection status-bundle-status resolver to fail on mismatched bundle_status_output_path" >&2
  exit 1
fi
...
if python3 "${ROOT_DIR}/planningops/scripts/resolve_monday_agent_harness_projection_status_bundle_status.py" \
  --status-validation-file "${BROKEN_STATUS_VALIDATION}" >/dev/null 2>&1; then
  echo "expected monday projection status-bundle-status resolver to fail on mismatched status_next_step" >&2
  exit 1
fi

echo "resolve monday agent harness projection status bundle status ok"
