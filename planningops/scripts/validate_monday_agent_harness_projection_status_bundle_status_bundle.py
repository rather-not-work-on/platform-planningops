...
import resolve_monday_agent_harness_projection_status_bundle_status as status_bundle_status_resolver
import validate_monday_agent_harness_projection as projection_validation


DEFAULT_BUNDLE = status_bundle_status_resolver.DEFAULT_OUTPUT
DEFAULT_SCHEMA = status_bundle_status_resolver.DEFAULT_BUNDLE_SCHEMA
DEFAULT_STATUS_SCHEMA = status_bundle_status_resolver.DEFAULT_STATUS_SCHEMA
DEFAULT_STATUS_VALIDATION_SCHEMA = status_bundle_status_resolver.DEFAULT_STATUS_VALIDATION_SCHEMA
DEFAULT_OUTPUT = (
    status_bundle_status_resolver.WORKSPACE_ROOT
...
def validate_semantics(
    ...
    *,
    ...
    if doc.get("resolved_status_bundle_status_bundle_path") != str(bundle_path.resolve()):
        append_unique(errors, "resolved_status_bundle_status_bundle_path must match the validated bundle file path")

    bundle_status_path = resolve_doc_path(doc.get("bundle_status_path"), base=bundle_path.parent)
    bundle_status_validation_path = resolve_doc_path(doc.get("bundle_status_validation_path"), base=bundle_path.parent)
    projection_status_bundle_path = doc.get("bundle_path")
    ...
    for key, value in (
        ...
        ("bundle_path", projection_status_bundle_path),
    ...
    if bundle_status_path is not None:
        try:
            canonical_bundle = status_bundle_status_resolver.resolve_status_bundle_status_bundle(
                artifact_file=None,
                status_file=str(bundle_status_path),
    ...
    if isinstance(bundle_status_report, dict):
        if bundle_status_path is not None and resolve_doc_path(
            bundle_status_report.get("bundle_status_output_path"), base=bundle_path.parent
        ) != bundle_status_path:
            append_unique(errors, "bundle_status_report.bundle_status_output_path must match bundle_status_path")
        if bundle_status_validation_path is not None and resolve_doc_path(
            bundle_status_report.get("bundle_status_validation_output_path"), base=bundle_path.parent
        ) != bundle_status_validation_path:
            append_unique(
                errors,
                "bundle_status_report.bundle_status_validation_output_path must match bundle_status_validation_path",
            )
        for bundle_field, report_field in (
            ...
            if doc.get(bundle_field) != bundle_status_report.get(report_field):
                append_unique(errors, f"{bundle_field} must match bundle_status_report.{report_field}")

    if isinstance(bundle_status_validation_report, dict):
        if bundle_status_path is not None and resolve_doc_path(
            bundle_status_validation_report.get("bundle_status_path"), base=bundle_path.parent
        ) != bundle_status_path:
            append_unique(errors, "bundle_status_validation_report.bundle_status_path must match bundle_status_path")
        if bundle_status_path is not None and resolve_doc_path(
            bundle_status_validation_report.get("bundle_status_output_path"), base=bundle_path.parent
        ) != bundle_status_path:
            append_unique(
                errors,
        ...
        if bundle_status_validation_path is not None and resolve_doc_path(
            bundle_status_validation_report.get("bundle_status_validation_output_path"), base=bundle_path.parent
        ) != bundle_status_validation_path:
            append_unique(
                errors,
                "bundle_status_validation_report.bundle_status_validation_output_path must match bundle_status_validation_path",
            )
        if bundle_status_validation_path is not None and resolve_doc_path(
            bundle_status_validation_report.get("output_path"), base=bundle_path.parent
        ) != bundle_status_validation_path:
            append_unique(errors, "bundle_status_validation_report.output_path must match bundle_status_validation_path")
        for bundle_field, report_field in (
            ("bundle_path", "bundle_path"),
            ("resolved_status_bundle_path", "resolved_status_bundle_path"),
            ("bundle_validation_output_path", "bundle_validation_output_path"),
            ...
            if doc.get(bundle_field) != bundle_status_validation_report.get(report_field):
                append_unique(errors, f"{bundle_field} must match bundle_status_validation_report.{report_field}")
    ...
    if canonical_bundle is not None:
        ...
        if doc != expected_bundle:
            append_unique(
                errors,
                "bundle must match canonical monday harness projection status-bundle-status bundle resolution",
            )

    report: dict[str, Any] | None = None
    if not errors:
        report = {
            "status_bundle_status_bundle_path": str(bundle_path.resolve()),
            "schema_path": str(bundle_schema_path.resolve()),
            "output_path": str(output_path.resolve()),
            "bundle_status_path": str(bundle_status_path.resolve()) if bundle_status_path is not None else None,
            "bundle_status_validation_path": (
                str(bundle_status_validation_path.resolve()) if bundle_status_validation_path is not None else None
            ),
            "bundle_path": projection_status_bundle_path,
            "resolved_status_bundle_path": resolved_status_bundle_path,
            "bundle_validation_output_path": bundle_validation_output_path,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate monday agent harness projection status-bundle-status bundle."
    )
