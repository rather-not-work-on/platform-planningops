...
SCHEMA_PATH="${ROOT_DIR}/planningops/schemas/monday-agent-harness-projection-status-bundle-status-bundle-status-bundle-status.schema.json"
INVALID_STATUS="${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status.json"
INVALID_STATUS_VALIDATION="${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status-validation.json"
...
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","finalPhase":"publish_evidence","finalStatus":"succeeded","verificationVerdict":"pass","completedAtUtc":"2026-03-23T13:40:00Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/readiness-projection.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","readinessStatus":"ready","reason":"verification_passed","verificationVerdict":"pass","blockingConditions":[],"requiredEvidenceRefs":["${EVIDENCE}"],"generatedAtUtc":"2026-03-23T13:40:01Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/verification-projection.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","verificationVerdict":"pass","verificationReportRefs":["${EVIDENCE}"],"failedChecks":[],"repairAttempts":0,"generatedAtUtc":"2026-03-23T13:40:02Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/operator-handoff-summary.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","finalStatus":"succeeded","verificationVerdict":"pass","handoffStatus":"not_required","handoffReason":"none","nextRequiredActor":"none","recommendedNextStep":"none","blockingQuestionSet":[],"generatedAtUtc":"2026-03-23T13:40:03Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
...
python3 "${ROOT_DIR}/planningops/scripts/validate_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status.py" \
  --status-file "${NEXT_STATUS}" \
  --output "${NEXT_STATUS_VALIDATION}" \
  --schema-file "${SCHEMA_PATH}" \
  --strict >/dev/null

python3 - <<'PY' "${ROOT_DIR}" "${NEXT_STATUS}" "${NEXT_STATUS_VALIDATION}" "${SCHEMA_PATH}" "${NEXT_BUNDLE}" "${NEXT_BUNDLE_VALIDATION}"
import json
import sys
...
python3 - <<'PY' "${NEXT_STATUS}" "${INVALID_STATUS}"
import json
import sys
from pathlib import Path
...
if [[ "${rc}" -eq 0 ]]; then
  echo "expected strict failure for invalid monday harness projection status bundle status bundle status bundle status"
  exit 1
fi
...
assert any("resolved_status_bundle_status_bundle_status_bundle_path must match bundle_path" in item for item in report["errors"]), report
assert any("ready must be false when verdict=fail" in item or "verdict must match propagated monday projection status-bundle-status-bundle-status-bundle aggregation" in item for item in report["errors"]), report
PY

echo "validate monday agent harness projection status bundle status bundle status bundle status contract ok"
