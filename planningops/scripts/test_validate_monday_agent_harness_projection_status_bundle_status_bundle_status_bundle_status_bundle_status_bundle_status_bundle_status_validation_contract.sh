...
NEW_STATUS="${TMP_DIR}/projection-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status.json"
NEW_STATUS_VALIDATION="${TMP_DIR}/projection-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.json"
SCHEMA_PATH="${ROOT_DIR}/planningops/schemas/monday-agent-harness-projection-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.schema.json"
INVALID_STATUS="${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status.json"
INVALID_STATUS_VALIDATION="${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.json"
...
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","finalPhase":"publish_evidence","finalStatus":"succeeded","verificationVerdict":"pass","completedAtUtc":"2026-03-23T15:20:00Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/readiness-projection.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","readinessStatus":"ready","reason":"verification_passed","verificationVerdict":"pass","blockingConditions":[],"requiredEvidenceRefs":["${EVIDENCE}"],"generatedAtUtc":"2026-03-23T15:20:01Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/verification-projection.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","verificationVerdict":"pass","verificationReportRefs":["${EVIDENCE}"],"failedChecks":[],"repairAttempts":0,"generatedAtUtc":"2026-03-23T15:20:02Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
cat >"${PROJECTION_ROOT}/operator-handoff-summary.json" <<JSON
{"missionId":"mh60-mission","runId":"mh60-run","sessionId":"mh60-session","finalStatus":"succeeded","verificationVerdict":"pass","handoffStatus":"not_required","handoffReason":"none","nextRequiredActor":"none","recommendedNextStep":"none","blockingQuestionSet":[],"generatedAtUtc":"2026-03-23T15:20:03Z","evidenceBundlePath":"${EVIDENCE}"}
JSON
...
python3 "${ROOT_DIR}/planningops/scripts/validate_monday_agent_harness_projection_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --status-file "${NEW_STATUS}" \
  --output "${NEW_STATUS_VALIDATION}" \
  --strict >/dev/null

python3 - <<'PY' "${ROOT_DIR}" "${NEW_STATUS_VALIDATION}" "${SCHEMA_PATH}" "${NEW_STATUS}" "${FINAL_BUNDLE}" "${FINAL_BUNDLE_VALIDATION}"
import json
import sys
...
doc["bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_validation_output_path"] = str(
    Path(sys.argv[2]).with_name("wrong-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.json").resolve()
)
...
if [[ "${rc}" -eq 0 ]]; then
  echo "expected strict failure for invalid monday harness projection status bundle status bundle status bundle status bundle status bundle status bundle status validation" >&2
  exit 1
fi
...
echo "validate monday agent harness projection status bundle status bundle status bundle status bundle status bundle status bundle status validation contract ok"
