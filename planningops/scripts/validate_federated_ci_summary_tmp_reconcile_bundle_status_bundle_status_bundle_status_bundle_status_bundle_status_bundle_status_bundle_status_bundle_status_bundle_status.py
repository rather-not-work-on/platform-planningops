...
from doctor_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle_status_bundle import (
    build_status,
)
    ...
    *,
    ...
    for field in (
        ...
        if status_doc.get(field) != expected_status.get(field):
            append_unique(
                errors,
                f"{field} must match canonical status-bundle-status bundle status bundle status bundle status doctor status",
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate federated CI tmp-summary reconcile bundle status-bundle-status bundle status bundle status bundle status bundle status artifacts."
    )
