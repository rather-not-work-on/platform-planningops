...
cat >"$REPORT_PATH" <<'JSON'
{
  ...
  "run_id": "reconcile-bundle-status-bundle-status-sample",
...
cat >"$VALIDATION_PATH" <<'JSON'
{
  ...
  "reconcile_run_id": "reconcile-bundle-status-bundle-status-sample",
...
assert report["status_run_id"] == "reconcile-bundle-status-bundle-status-sample", report
...
if [[ "$rc" -eq 0 ]]; then
  echo "expected strict failure for invalid tmp-summary reconcile bundle status-bundle status"
  exit 1
fi
...
assert any("ready must match canonical status-bundle doctor status" in item for item in report["errors"]), report
assert any("next_step must match canonical status-bundle doctor status" in item for item in report["errors"]), report
PY

echo "validate federated ci summary tmp reconcile bundle status bundle status contract ok"
