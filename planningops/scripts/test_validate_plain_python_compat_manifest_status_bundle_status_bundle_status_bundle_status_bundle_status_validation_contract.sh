...
SCHEMA_PATH="${ROOT_DIR}/planningops/schemas/plain-python-compat-manifest-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.schema.json"
...
python3 "${ROOT_DIR}/planningops/scripts/validate_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --status-file "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH}" \
  --output "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH}" \
  --strict

python3 - <<'PY' "${ROOT_DIR}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH}" "${SCHEMA_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH}"
import json
import sys
...
assert report["output_path"] == str(Path(report_path).resolve()), report
assert report["bundle_status_bundle_status_bundle_status_bundle_status_path"] == str(Path(status_path).resolve()), report
assert report["bundle_status_bundle_status_bundle_status_bundle_output_path"] == str(Path(status_path).resolve()), report
assert report["bundle_status_bundle_status_bundle_status_bundle_validation_output_path"] == str(Path(report_path).resolve()), report
assert report["bundle_validation_output_path"] == str(Path(bundle_validation_path).resolve()), report
...
python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH}" "${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status-bundle-status.json"
import json
import sys
from pathlib import Path
...
doc["ready"] = False
doc["next_step"] = "none"
doc["bundle_validation_output_path"] = str(Path(sys.argv[2]).with_name("missing-validation.json").resolve())
doc["bundle_status_bundle_status_bundle_status_bundle_output_path"] = str(Path(sys.argv[2]).with_name("wrong-status.json").resolve())
doc.pop("bundle_status_bundle_status_bundle_status_bundle_validation_output_path")
Path(sys.argv[2]).write_text(json.dumps(doc, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
PY

set +e
python3 "${ROOT_DIR}/planningops/scripts/validate_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status_bundle_status.py" \
  --status-file "${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status-bundle-status.json" \
  --output "${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.json" \
  --strict
rc=$?
set -e

if [[ "${rc}" -eq 0 ]]; then
  echo "expected strict failure for invalid plain python compat manifest status bundle status bundle status bundle status bundle status"
  exit 1
fi

python3 - <<'PY' "${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status-bundle-status-validation.json"
import json
import sys
from pathlib import Path
...
assert any("bundle_status_bundle_status_bundle_status_bundle_output_path must match the validated status file path" in item for item in report["errors"]), report
assert any("bundle_status_bundle_status_bundle_status_bundle_validation_output_path must match the validation output path" in item for item in report["errors"]), report
...
echo "validate plain python compat manifest status bundle status bundle status bundle status bundle status validation contract ok"
