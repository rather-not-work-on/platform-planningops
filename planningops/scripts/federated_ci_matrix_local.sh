...
run_check "loop-guardrails" "policy" \
  bash -lc "cd '${ROOT_DIR}' && python3 planningops/scripts/run_track1_gate_dryrun.py --kpi-path planningops/fixtures/track1-kpi-baseline-ci.json --strict && bash planningops/scripts/test_track1_gate_dryrun_contract.sh && bash planningops/scripts/test_bootstrap_two_track_backlog_contract.sh && bash planningops/scripts/test_validate_project_field_schema_matrix.sh && bash planningops/scripts/test_module_readme_contract.sh && bash planningops/scripts/test_normalize_ready_implementation_blueprint_refs.sh && bash planningops/scripts/test_issue_loop_runner_multi_repo_intake.sh && bash planningops/scripts/test_loop_checkpoint_resume.sh && bash planningops/scripts/test_lease_lock_watchdog.sh && bash planningops/scripts/test_escalation_gate.sh"
