...
STATUS_SCHEMA_PATH="${ROOT_DIR}/planningops/schemas/plain-python-compat-manifest-status-bundle-status-bundle-status-bundle-status.schema.json"
...
python3 - <<'PY' "${ROOT_DIR}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH}" "${STATUS_SCHEMA_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_VALIDATION_PATH}"
import json
import sys
...
assert report["bundle_status_bundle_status_bundle_validation_output_path"] == str(status_validation_path.resolve()), report
assert report["bundle_path"] == str(bundle_path.resolve()), report
assert report["resolved_status_bundle_status_bundle_status_bundle_path"] == str(status_path.resolve()), report
...
doc.pop("bundle_status_bundle_status_bundle_output_path")
...
python3 - <<'PY' "${ROOT_DIR}" "${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status.json" "${STATUS_SCHEMA_PATH}"
import json
import sys
from pathlib import Path
...
assert any("$.bundle_status_bundle_status_bundle_output_path is required" in item for item in errors), errors
assert any("$.bundle_status_bundle_status_bundle_validation_output_path is required" in item for item in errors), errors
PY

echo "validate plain python compat manifest status bundle status bundle status bundle status contract ok"
