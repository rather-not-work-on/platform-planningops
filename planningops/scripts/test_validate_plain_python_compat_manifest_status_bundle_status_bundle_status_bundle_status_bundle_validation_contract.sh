...
SCHEMA_PATH="${ROOT_DIR}/planningops/schemas/plain-python-compat-manifest-status-bundle-status-bundle-status-bundle-status-bundle-validation.schema.json"
...
python3 "${ROOT_DIR}/planningops/scripts/validate_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status_bundle.py" \
...
python3 - <<'PY' "${ROOT_DIR}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH}" "${SCHEMA_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH}" "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_PATH}"
import json
import sys
...
python3 - <<'PY' "${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH}" "${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status-bundle.json"
import json
import sys
from pathlib import Path
...
doc["bundle_status_bundle_status_bundle_validation_report"]["bundle_status_bundle_status_bundle_output_path"] = str(
    Path(sys.argv[2]).with_name("wrong-status-bundle-status-bundle-status-bundle-status.json").resolve()
)
...
set +e
python3 "${ROOT_DIR}/planningops/scripts/validate_plain_python_compat_manifest_status_bundle_status_bundle_status_bundle_status_bundle.py" \
  --bundle-file "${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status-bundle.json" \
  --output "${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status-bundle-validation.json" \
  --strict
rc=$?
set -e

if [[ "${rc}" -eq 0 ]]; then
  echo "expected strict failure for invalid plain python compat manifest status bundle status bundle status bundle status bundle"
  exit 1
fi

python3 - <<'PY' "${TMP_DIR}/invalid-status-bundle-status-bundle-status-bundle-status-bundle-validation.json"
import json
import sys
from pathlib import Path
...
assert any("bundle_status_bundle_status_bundle_validation_report.bundle_status_bundle_status_bundle_output_path must match bundle_status_bundle_status_bundle_status_path" in item for item in report["errors"]), report
assert any("bundle must match canonical status-bundle-status-bundle-status-bundle-status bundle resolution" in item for item in report["errors"]), report
PY

echo "validate plain python compat manifest status bundle status bundle status bundle status bundle validation contract ok"
