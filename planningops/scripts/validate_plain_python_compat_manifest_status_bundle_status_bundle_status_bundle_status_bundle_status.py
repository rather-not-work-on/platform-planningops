...
def validate_semantics(status_path: Path, doc: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    bundle_status_output_path = doc.get("bundle_status_bundle_status_bundle_status_bundle_output_path")
    ...
    if bundle_status_output_path != str(status_path.resolve()):
        append_unique(
            errors,
            "bundle_status_bundle_status_bundle_status_bundle_output_path must match the validated status file path",
        )
    if not isinstance(bundle_status_validation_output_path, str) or not bundle_status_validation_output_path.strip():
        append_unique(
            errors,
            "bundle_status_bundle_status_bundle_status_bundle_validation_output_path must be a non-empty string",
        )

    for key, value in (
        ("bundle_path", bundle_path),
        ("resolved_status_bundle_status_bundle_status_bundle_status_bundle_path", resolved_bundle_path),
    ...
    if (
        ...
        append_unique(
            errors,
            "resolved_status_bundle_status_bundle_status_bundle_status_bundle_path must match bundle_status_bundle_status_bundle_status_bundle_output_path",
        )
    ...
    if isinstance(validation_doc, dict) and validation_path is not None:
        ...
        if (
            ...
            != validation_doc.get("bundle_status_bundle_status_bundle_status_path")
        ):
            append_unique(
                errors,
        ...
        if (
            bundle_status_bundle_status_bundle_validation_path
            != validation_doc.get("bundle_status_bundle_status_bundle_validation_path")
        ):
            append_unique(
                errors,
...
def build_validation_report(status_path: Path, schema_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    if doc.get("bundle_status_bundle_status_bundle_status_bundle_validation_output_path") != str(output_path.resolve()):
        append_unique(
            errors,
            "bundle_status_bundle_status_bundle_status_bundle_validation_output_path must match the validation output path",
        )
        verdict = "fail"
    ...
    return {
        ...
        "bundle_status_bundle_status_bundle_status_bundle_output_path": doc.get(
            "bundle_status_bundle_status_bundle_status_bundle_output_path"
        ),
        "bundle_status_bundle_status_bundle_status_bundle_validation_output_path": doc.get(
            "bundle_status_bundle_status_bundle_status_bundle_validation_output_path"
        ),
        "bundle_validation_output_path": doc.get("bundle_validation_output_path"),
        "bundle_status_bundle_status_bundle_status_path": doc.get("bundle_status_bundle_status_bundle_status_path"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate plain python compatibility manifest status-bundle-status-bundle-status-bundle-status-bundle status report."
    )
