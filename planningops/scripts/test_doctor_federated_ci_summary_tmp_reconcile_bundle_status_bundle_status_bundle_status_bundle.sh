...
BROKEN_BUNDLE_PATH="$TMP_DIR/federated-ci-summary-tmp-reconcile-bundle-status-bundle-status-bundle-status-bundle-broken.json"
REPORT_OUTPUT="$TMP_DIR/doctor-status-bundle-status-bundle-status-bundle.txt"
BROKEN_REPORT_OUTPUT="$TMP_DIR/doctor-status-bundle-status-bundle-status-bundle-broken.txt"
...
cat >"$REPORT_PATH" <<'JSON'
{
  ...
  "run_id": "reconcile-bundle-status-bundle-status-bundle-status-bundle-doctor-sample",
...
cat >"$VALIDATION_PATH" <<'JSON'
{
  ...
  "reconcile_run_id": "reconcile-bundle-status-bundle-status-bundle-status-bundle-doctor-sample",
...
grep -q "bundle path: ${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_REAL}" "$REPORT_OUTPUT"
grep -q "bundle validation output path: ${STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_REAL}" "$REPORT_OUTPUT"
...
grep -q "run_id: reconcile-bundle-status-bundle-status-bundle-status-bundle-doctor-sample" "$REPORT_OUTPUT"
...
python3 - <<'PY' "$STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_PATH" "$STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH" "$DOCTOR_STATUS_PATH" "$DOCTOR_STATUS_VALIDATION_PATH"
import json
import sys
from pathlib import Path
...
assert bundle["run_id"] == "reconcile-bundle-status-bundle-status-bundle-status-bundle-doctor-sample", bundle
...
assert validation["run_id"] == "reconcile-bundle-status-bundle-status-bundle-status-bundle-doctor-sample", validation
...
assert status["run_id"] == "reconcile-bundle-status-bundle-status-bundle-status-bundle-doctor-sample", status
...
assert status_validation["status_run_id"] == "reconcile-bundle-status-bundle-status-bundle-status-bundle-doctor-sample", status_validation
...
if python3 "$ROOT_DIR/scripts/doctor_federated_ci_summary_tmp_reconcile_bundle_status_bundle_status_bundle_status_bundle.py" \
  --bundle-file "$BROKEN_BUNDLE_PATH" \
  --bundle-validation-output "$STATUS_BUNDLE_STATUS_BUNDLE_STATUS_BUNDLE_VALIDATION_PATH" \
  --status-output "$DOCTOR_STATUS_PATH" \
  --status-validation-output "$DOCTOR_STATUS_VALIDATION_PATH" \
  --require-pass >"$BROKEN_REPORT_OUTPUT" 2>&1; then
  echo "expected tmp-summary reconcile bundle status-bundle-status-bundle-status-bundle doctor to fail on broken bundle"
  exit 1
fi
...
echo "doctor federated ci summary tmp reconcile bundle status bundle status bundle status bundle ok"
