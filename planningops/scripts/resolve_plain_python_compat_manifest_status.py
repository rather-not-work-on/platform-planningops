...
import resolve_plain_python_compat_manifest as manifest_resolver
from validate_plain_python_compat_manifest import load_json, validate_schema


WORKSPACE_ROOT = manifest_resolver.repo_root()
DEFAULT_STATUS = WORKSPACE_ROOT / "planningops/artifacts/validation/plain-python-compat-manifest-status.json"
DEFAULT_STATUS_VALIDATION = (
    WORKSPACE_ROOT / "planningops/artifacts/validation/plain-python-compat-manifest-status-validation.json"
)
DEFAULT_STATUS_SCHEMA = WORKSPACE_ROOT / "planningops/schemas/plain-python-compat-manifest-status.schema.json"
DEFAULT_STATUS_VALIDATION_SCHEMA = (
    WORKSPACE_ROOT / "planningops/schemas/plain-python-compat-manifest-status-validation.schema.json"
)
DEFAULT_BUNDLE_SCHEMA = WORKSPACE_ROOT / "planningops/schemas/plain-python-compat-manifest-status-bundle.schema.json"
DEFAULT_OUTPUT = WORKSPACE_ROOT / "planningops/artifacts/validation/plain-python-compat-manifest-status-bundle.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Resolve the canonical plain-python manifest status + status-validation sidecar pair."
    )
...
def is_status_doc(doc: dict) -> bool:
    return all(key in doc for key in ("status_output_path", "status_validation_output_path", "ready", "next_step"))


def is_status_validation_doc(doc: dict) -> bool:
    return all(
        key in doc
        for key in ("status_path", "status_output_path", "status_validation_output_path", "output_path", "status_ready")
    )
...
def resolve_from_status(
    ...
    *,
    ...
    validate_doc_schema(status_doc, status_schema_path, "plain python manifest status report")

    embedded_status_path = resolve_doc_path(status_doc.get("status_output_path"), base=status_path.parent)
    if embedded_status_path != status_path:
        raise SystemExit("status_output_path must match the resolved status report path")
    ...
    validate_doc_schema(
        ...
        "plain python manifest status-validation report",
    )
    ...
    if (
        resolve_doc_path(status_validation_doc.get("status_output_path"), base=status_validation_path.parent)
        != status_path
    ):
    ...
    if status_validation_doc.get("resolved_guardrail_script_paths") != status_doc.get("resolved_guardrail_script_paths"):
        raise SystemExit("status-validation report resolved_guardrail_script_paths must match the status report")
...
def resolve_from_status_validation(
    ...
    *,
    ...
    if resolve_doc_path(status_doc.get("status_output_path"), base=status_path.parent) != status_path:
        raise SystemExit("status report status_output_path must match the resolved status report path")
    ...
    *,
    ...
    if status_file:
        ...
        if is_status_doc(artifact_doc):
            ...
            raise SystemExit("artifact is neither a status report nor a status-validation report")

    output_path = resolve_output_path(output)
    if output_path is not None:
        bundle = dict(bundle)
        bundle["resolved_status_bundle_path"] = str(output_path)
    validate_doc_schema(bundle, bundle_schema_path, "plain python manifest status bundle")
