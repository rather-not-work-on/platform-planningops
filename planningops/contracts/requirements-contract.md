...
3. Selector must deterministically assign `loop_profile` from issue context and transition history (`L1|L2|L3|L4|L5`).
...
7. Runtime and provider policy must be resolvable by task key (`issue-<number>` with `default` fallback).
8. Replanning trigger must be raised when `inconclusive` repeats twice or same `reason_code` repeats three times for the same issue.
9. Runner must resolve a repo execution adapter by `target_repo` and invoke `before_loop` and `after_loop` hooks.
10. Adapter hook result payload must satisfy required fields (`status`, `phase`, `adapter`, `target_repo`, `reason_code`, `message`).
11. Adapter reason taxonomy must be constrained to `contract|permission|context|runtime|feedback_failed`.
12. Attempt budget must be derivable from issue body with defaults (`max_attempts`, `max_duration_minutes`, `max_token_budget`).
13. Invalid attempt budget values must be treated as non-selectable input until corrected.
14. Runner must persist checkpoint stages (`pre_hook`, `loop_executed`, `verified`, `feedback_applied`) per issue.
15. Runner must support resume mode that reuses valid checkpoint artifacts.
16. Runner must enforce per-issue lease lock and reject concurrent duplicate execution.
17. Runner must emit watchdog report for lock lifecycle and stale lock recovery.
18. Escalation gate must trigger auto-pause on `same_reason_x3` or `inconclusive_x2`.
19. Auto-pause trigger must emit replan decision artifact and transition-log event.
